 ...
 * "timeframe:structure:roomUpPct:roomDownPct:extension" string per timeframe (e.g.
 * "1h:up:0.42:0.18:low"). Room values round to 2 decimals here (full precision stays on
 * symbols.<SYM>.geometryContext). A missing individual field is an empty position
 * ("1h:up::0.18:", positions still line up left to right); a timeframe with no geometry
 * at all collapses to one token, "1h:na", instead of four. Phase 11: stays well under
 * 300 bytes per symbol.
...
export function geometryTraceSummary(geometryByTf) {
  ...
  return Object.entries(geometryByTf || {}).map(([tf, g]) => (g
    ? `${tf}:${g.structure || ''}:${field(g.roomToNextResistance)}:${field(g.roomToNextSupport)}:${(g.extensionRisk && g.extensionRisk.level) || ''}`
    : `${tf}:na`));
}
...
/**
 * Near-miss diagonals (phase 9): sides where no line is detected but one passes every
 * fitDiagonal gate with exactly `diagonalMinTouches - 1` touches - a line one touch short
 * of being published. Feeds the visual gate only; never published as a line and never
 * added to geometryContext, so phase 7/8 outputs are unchanged.
 * @param {{highs:Array, lows:Array}} pivots - swingPivots(...) output
 * @param {Object} [cfg=ENGINE_CONFIG.geometry]
 * @param {{candles:Array<Object>, atr:number}} ctx - same as fitDiagonal
 * @returns {{count:number, sides:Array<'support'|'resistance'>}}
 */
export function nearMissDiagonals(pivots, cfg = ENGINE_CONFIG.geometry, ctx = {}) {
  const sides = [];
  const nearMiss = cfg.diagonalMinTouches - 1;
  const { candles, atr: atrValue } = ctx;
  if (nearMiss >= 2 && pivots && Array.isArray(candles) && candles.length > 0 && candles.every(isValidCandle)
    && isFiniteNumber(atrValue) && atrValue > 0) {
    const looser = { ...cfg, diagonalMinTouches: nearMiss };
    for (const side of ['support', 'resistance']) {
      if (fitDiagonal(pivots, side, cfg, ctx).detected) continue;
      const line = fitDiagonal(pivots, side, looser, ctx);
      if (line.detected && line.touches === nearMiss) sides.push(side);
    }
  }
  return { count: sides.length, sides };
}
