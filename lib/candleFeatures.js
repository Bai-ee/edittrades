...
/**
 * Calculate wick dominance (upper vs lower wick analysis)
 * @param {Object} candle - OHLC candle
 * @returns {Object} Wick dominance analysis
 */
export function calculateWickDominance(candle) {
  if (!candle || !candle.open || !candle.high || !candle.low || !candle.close) {
    return null;
  }
  ...
  if (range === 0) {
    return {
      upperWickDominance: 0,
      lowerWickDominance: 0,
      dominantWick: 'NONE',
      wickRatio: 0
    };
  }
  ...
  let dominantWick = 'NONE';
  if (upperWickPct > 50 && upperWickPct > lowerWickPct * 1.5) {
    dominantWick = 'UPPER';
  } else if (lowerWickPct > 50 && lowerWickPct > upperWickPct * 1.5) {
    dominantWick = 'LOWER';
  } else if (upperWickPct > 30 && lowerWickPct > 30) {
    dominantWick = 'BOTH';
  }
  
  // Calculate wick ratio (larger wick / smaller wick)
  const wickRatio = upperWick > 0 && lowerWick > 0 
...
/**
 * Detect exhaustion signals from wick patterns
 * @param {Array} candles - Array of OHLC candles (last 3-5 candles)
 * @returns {Object} Exhaustion signal analysis
 */
export function detectExhaustionSignals(candles) {
  ...
  const recentCandles = candles.slice(-3); // Last 3 candles
  ...
  const latestWick = calculateWickDominance(latest);
  const prevWick = calculateWickDominance(previous);
  
  if (!latestWick || !prevWick) {
    return {
      exhaustionSignal: 'NONE',
      exhaustionType: null,
      confidence: 0
  ...
  // Lower wick rejection (bullish exhaustion of selling)
  if (latestWick.lowerWickDominance > 60 && latestWick.lowerWickSize > latestWick.bodySize * 2) {
    return {
      exhaustionSignal: 'LOWER_WICK_REJECTION',
      exhaustionType: 'BULLISH',
      confidence: latestWick.lowerWickDominance > 70 ? 0.85 : 0.65,
      wickSize: latestWick.lowerWickSize,
      bodySize: latestWick.bodySize
    };
  }

  // Upper wick rejection (bearish exhaustion of buying)
  if (latestWick.upperWickDominance > 60 && latestWick.upperWickSize > latestWick.bodySize * 2) {
    return {
      exhaustionSignal: 'UPPER_WICK_REJECTION',
      exhaustionType: 'BEARISH',
      confidence: latestWick.upperWickDominance > 70 ? 0.85 : 0.65,
      wickSize: latestWick.upperWickSize,
      bodySize: latestWick.bodySize
    };
  }

  // Double wick pattern (indecision/exhaustion)
  if (latestWick.upperWickDominance > 30 && latestWick.lowerWickDominance > 30 && 
      latestWick.bodySize < (latest.high - latest.low) * 0.3) {
    return {
      exhaustionSignal: 'DOUBLE_WICK_INDECISION',
      exhaustionType: 'NEUTRAL',
      confidence: 0.5,
      wickSize: Math.max(latestWick.upperWickSize, latestWick.lowerWickSize),
      bodySize: latestWick.bodySize
    };
  }
...
/**
 * Analyze body strength relative to range
 * @param {Object} candle - OHLC candle
 * @returns {Object} Body strength analysis
 */
export function analyzeBodyStrength(candle) {
  if (!candle || !candle.open || !candle.high || !candle.low || !candle.close) {
    return null;
  }
  ...
  if (range === 0) {
    return {
      bodyStrength: 0,
      bodyStrengthCategory: 'NONE',
      bodyToRangeRatio: 0
    };
  }

  const bodyToRangeRatio = (body / range) * 100;
  
  let bodyStrengthCategory = 'WEAK';
  if (bodyToRangeRatio > 70) {
    bodyStrengthCategory = 'VERY_STRONG';
  } else if (bodyToRangeRatio > 50) {
    bodyStrengthCategory = 'STRONG';
  } else if (bodyToRangeRatio > 30) {
    bodyStrengthCategory = 'MODERATE';
  }

  return {
    bodyStrength: parseFloat(bodyToRangeRatio.toFixed(2)),
    bodyStrengthCategory,
    bodyToRangeRatio: parseFloat(bodyToRangeRatio.toFixed(2)),
    bodySize: parseFloat(body.toFixed(2)),
