 ...
 * (default `note`); `engineRef` is an optional object of short strings. `source` is
 * stamped by the server, never read from the body: null for the GPT Action path,
 * 'telegram' for the Telegram bot's /log (T-1, docs/PLAN_TELEGRAM.md), 'execution' for
 * lib/execution/executor.js orders (T-3, docs/PLAN_TELEGRAM_EXECUTION.md).
 *
 * `execRef` (source 'execution' only; ignored for every other caller, so GPT and
 * Telegram records keep exactly RECORD_KEYS) links an execution entry to its order:
 * {positionIdHash, ticketNonce, openJournalId, fillSource, actionId}, short id-like
 * strings or null. It is appended after RECORD_KEYS when present.
 */
...
export const SOURCES = Object.freeze(['telegram', 'execution']);
...
export const EXEC_REF_FIELDS = Object.freeze(['positionIdHash', 'ticketNonce', 'openJournalId', 'fillSource', 'actionId']);
const EXEC_REF_RE = /^[A-Za-z0-9_-]{1,64}$/;
...
export const RECORD_KEYS = Object.freeze([
  ...
  ...POSITIVE_FIELDS, ...SIGNED_FIELDS, 'engineRef', 'text', 'source'
]);
 ...
 * @param {{now: Date|number|string, newId: () => string, source?: string|null}} ctx - `source`
 *   is the server's own label for the caller (SOURCES, or null); a body `source` is ignored
 * @returns {{ok: true, record: Object} | {ok: false, errors: string[]}}
 */
export function validateJournalEntry(body, { now, newId, source = null }) {
  const errors = [];
  ...
  let execRef = null;
  if (source === 'execution' && !absent(body.execRef)) {
    if (!isObj(body.execRef)) errors.push('execRef must be an object or null');
    else {
      execRef = {};
      for (const k of EXEC_REF_FIELDS) {
        const v = body.execRef[k];
        if (absent(v)) { execRef[k] = null; continue; }
        if (typeof v !== 'string' || !EXEC_REF_RE.test(v)) { errors.push(`execRef.${k} must be 1-64 characters of A-Z, a-z, 0-9, _ or -`); continue; }
        execRef[k] = v;
      }
    }
  }
