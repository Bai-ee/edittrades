...
export function calculateVolumeAnalysis(candles, period = 20) {
  ...
  if (!Array.isArray(candles) || candles.length < 1) {
    return defaultVolume; // Return default instead of null
  }

  // Get current volume (use last candle or 0)
  ...
  if (!hasVolume || currentVolume <= 0) {
    // Return default with last candle volume if available
    return {
