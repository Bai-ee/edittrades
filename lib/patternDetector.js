 ...
 * breakoutLevel/invalidation, via `sign * poleBase`.
 */
function measureImpulse(candles, flagStart, cfg) {
  ...
  return range > 0 ? { peakHigh: candles[peakIdx].high, range, poleBase: candles[troughIdx].low } : null;
}
 ...
 *   measuredTarget, measuredRR }`, or null when no flag exists. poleHeight/measuredTarget/
 *   measuredRR are computed from this call's breakoutLevel/invalidation (quick pass Q1);
 *   a caller that snaps those levels to geometry afterward must call measuredMoveFor(...)
 *   again with the snapped levels to keep them in step (services/scalpContext.js does).
 */
