 ...
 *
 * ATR is computed locally (Wilder, `flag.atrPeriod`). `lib/advancedIndicators.js`
 * `calculateATR` is not imported: plan rule 7 reserves that import for phase 7.
 */
...
/**
 * Wilder ATR per candle index. Direction-invariant (true range ignores sign).
...
/** Lowest oriented price still counted as on-side of EMA21 (tolerance band). */
function emaFloor(emaValue, cfg) {
  return emaValue - Math.abs(emaValue) * (cfg.wickTolerancePct / 100);
}
 ...
 * @returns {'hold'|'wick'|'acceptance'|null} null when EMA21 is missing for any candle
 */
function classifyEma21(candles, ema, from, to, cfg) {
  let run = 0;
  let pierced = false;
  for (let i = from; i <= to; i++) {
    if (ema[i] === null) return null;
    const floor = emaFloor(ema[i], cfg);
...
function findOrientedFlag(candles, ema, atr, cfg) {
  ...
  for (let flagEnd = lastIdx; flagEnd >= earliestEnd; flagEnd--) {
    ...
    for (let len = cfg.maxFlagCandles; len >= cfg.minCandles; len--) {
      ...
      if (ema[flagStart] === null || candles[flagStart].close < emaFloor(ema[flagStart], cfg)) continue;

      const hold = classifyEma21(candles, ema, flagStart, lastIdx, cfg);
 ...
 *   no flag exists
 */
export function detectFlag({ candles, ema21History, stochRsi = null }, direction, cfg = ENGINE_CONFIG.flag) {
  ...
  const atr = wilderAtr(oriented, cfg.atrPeriod);
...
export default { detectFlag, detectCandidateSetups, DIRECTIONS };
