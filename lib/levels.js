...
export function computeLevels(candles, currentPrice, swingPoints = {}, threshold = 0.5) {
  ...
  /**
   * Has the newest bar actually closed?
   *
   * Exchange OHLC responses include the bar still forming, and nothing
   * upstream drops it. So `latestCandle.close` is not a close at all — it is
   * the last trade inside an open bar, and on a 4h timeframe it can cross a
   * level and cross back repeatedly over four hours.
   *
   * A field named `brokeResistanceOnClose` asserting that about an unclosed
   * bar is the same class of defect as fabricated data: the name promises a
   * completed event and the value delivers a provisional one. The honest
   * answer while the bar is open is "not known yet", so the break flags become
   * null rather than false — false would read as "checked, did not break".
   */
  const barClosed = Number.isFinite(latestCandle?.closeTime)
    ? Date.now() >= latestCandle.closeTime
    : null;

  const brokeResistanceOnClose = barClosed === false
    ? null
    : (nearestResistance && previousCandle
        ? latestCandle.close > nearestResistance && previousCandle.close <= nearestResistance
        : false);

  const brokeSupportOnClose = barClosed === false
    ? null
    : (nearestSupport && previousCandle
        ? latestCandle.close < nearestSupport && previousCandle.close >= nearestSupport
        : false);
  ...
  return {
    ...
    brokeSupportOnClose,
    // null while the newest bar is still forming, so a consumer can tell
    // "did not break" from "cannot know yet".
    barClosed
  };
}
