...
/**
 * `room:blocked-<tf>`: a geometry resistance zone (long) or support zone (short) on any
 * geometry timeframe sits between breakoutLevel and measuredTarget - the measured-move
 * target has a level in the way before it gets there.
 */
function roomBlockedReasons(candidate, geometryContext) {
  ...
  for (const [tf, g] of Object.entries(geometryContext || {})) {
    ...
    if (blocked) codes.push(`room:blocked-${tf}`);
  }
  return codes;
}
