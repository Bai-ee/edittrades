...
/**
 * One compact string for decisionTrace (always published, ≤ 120 bytes without the
 * quick-pass Q3 suffix):
 ...
 *
 * Quick pass Q3: when `topDown` and `above200` (lib/topDown.js outputs) are supplied,
 * two more tokens are appended - `|td:<bull|bear|mixed>:<aligned>/4|a200:<count>/<of>` -
 * e.g. `...|ct:1|td:bull:3/4|a200:5/7`. Omitted (identical to the old string) when either
 * argument is null, so every existing caller is unaffected.
 */
export function biasTraceSummary(matrix, decisionInputs, alignment, topDown = null, above200 = null) {
  ...
  let out = `scalp:${h(decisionInputs.directionalBias.scalp)}|swing:${h(decisionInputs.directionalBias.swing)}|tf:${tfs}|ct:${ct}`;
  if (topDown) out += `|td:${topDown.sentiment}:${topDown.aligned}/4`;
  if (above200) out += `|a200:${above200.count}/${above200.of}`;
  return out;
}
