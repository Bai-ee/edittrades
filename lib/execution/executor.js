...
import { readRiskPolicyConfig, applyRiskPrefs, evaluateRiskPolicy, riskOverrideBound } from './riskPolicy.js';
import {
...
/** Wallet-equity read cache (T-8: an RPC round trip per preflight would be wasteful). */
export const EQUITY_CACHE_MS = 60_000;
...
export const CLOSE_VERIFY_POLL_MS = 1000;
export const CLOSE_VERIFY_MAX_MS = 60_000; // keeper fill wait for a decrease request, same budget as waitForFill
export const EMERGENCY_CLOSE_RETRY_MS = 5000;
export const EMERGENCY_CLOSE_MAX_MS = 45_000; // with DEFAULT_LAND_MAX_WAIT_MS 45 s the whole open fits the webhook's 300 s maxDuration
...
/**
 * Simulation failure -> Error whose message carries the program error and the last logs.
 * @solana/kit RPC results hold BigInt, so a plain JSON.stringify threw ("Do not know how to
 * serialize a BigInt", 2026-09-26) and hid the real reason from the card and the audit.
 */
const simFail = (prefix, sim) => {
  const err = JSON.stringify(sim && sim.err, (k, v) => (typeof v === 'bigint' ? v.toString() : v));
  const tail = sim && Array.isArray(sim.logs) && sim.logs.length ? ` | ${sim.logs.slice(-4).join(' | ')}` : '';
  return new Error(`${prefix}: ${err}${tail}`);
};
...
/**
 * Realized PnL (USD, signed) over the last `days` calendar days (today = day 0), from
 * journal closes + audit closes not journaled (T-8: generalized from the old single-day
 * dailyLossUsd so the risk policy can read a weekly window with the same logic).
 */
export async function pnlWindow(store, nowMs, days) {
  try {
    const dayStrs = Array.from({ length: Math.max(1, days) }, (_, d) => new Date(nowMs - d * 86_400_000).toISOString().slice(0, 10));
    const [journals, audits] = await Promise.all([
      Promise.all(dayStrs.map((day) => readBlob(store.get, journalDayPath(day)))),
      Promise.all(dayStrs.map((day) => readBlob(store.get, auditDayPath(day))))
    ]);
    const journalIds = new Set();
    let pnl = 0;
    for (const j of journals) {
      for (const r of parseJsonlObjects(j && j.text)) {
        if (r.kind === 'close' && isNum(r.resultUsd)) { pnl += r.resultUsd; journalIds.add(r.id); }
      }
    }
    ...
    return { ok: true, pnlUsd: r2(pnl) };
  } catch (err) {
    return { ok: false, pnlUsd: null, error: errName(err) };
  }
}

/** Realized loss today (USD, >= 0). Thin wrapper over pnlWindow(store, nowMs, 1). */
export async function dailyLossUsd(store, nowMs) {
  const r = await pnlWindow(store, nowMs, 1);
  return r.ok ? { ok: true, lossUsd: r2(Math.max(0, -r.pnlUsd)) } : { ok: false, lossUsd: null, error: r.error };
}
...
export function createExecutor(deps = {}) {
  ...
  const getAccountSnapshot = deps.getAccountSnapshot || (async (opts) => {
    const wt = await import('../../services/walletTracker.js');
    return wt.getAccountSnapshot(opts);
  });
  ...
  // ------------------------------------------------------------------ wallet equity (T-8)
  //
  // Equity = the SIGNING wallet's own value (SOL + stables), read via walletTracker.js's
  // snapshot logic pointed at an explicit `address` (never TRACKED_WALLET_ADDRESS, never a
  // key) -- plus open-position collateral +/- unrealized PnL, added on top per call since
  // readPositions() is already fresh there. Cached 60 s: an RPC round trip per preflight
  // would be wasteful, and the risk numbers do not need to be perfectly live.

  let equityCache = null; // { atMs, baseEquityUsd, equitySource, freeGasSol }

  /** BTC|ETH|SOL -> USD, for pricing the SOL holding. Same sources as liveFill. */
  async function equityPrices() {
    if (deps.getMarkPrices) {
      try { const p = await deps.getMarkPrices(); if (p && typeof p === 'object') return p; } catch { /* fall through */ }
    }
    try {
      const payload = await buildContext({ symbols: SYMBOLS });
      const syms = payload && payload.symbols ? payload.symbols : {};
      const out = {};
      for (const sym of SYMBOLS) { const f = fillFromSymbol(syms[sym]); if (f) out[sym] = f.price; }
      return out;
    } catch { return {}; }
  }

  /** The wallet-only baseline (SOL + stables), fresh (no cache). null equity = unreadable. */
  async function readWalletEquity() {
    let addr = null;
    try { addr = await walletAddress(); } catch { addr = null; }
    if (!addr) return { baseEquityUsd: null, equitySource: 'wallet_unavailable', freeGasSol: null };
    let snap = null;
    try { snap = await getAccountSnapshot({ address: addr, prices: await equityPrices() }); } catch { snap = null; }
    if (!snap || snap.status === 'unavailable' || snap.status === 'disabled') {
      return { baseEquityUsd: null, equitySource: (snap && snap.address) || 'wallet_unavailable', freeGasSol: null };
    }
    const solHolding = Array.isArray(snap.holdings) ? snap.holdings.find((h) => h && h.asset === 'SOL') : null;
    const baseEquityUsd = (isNum(snap.margin && snap.margin.usd) ? snap.margin.usd : 0) + (solHolding && isNum(solHolding.usdValue) ? solHolding.usdValue : 0);
    return { baseEquityUsd: r2(baseEquityUsd), equitySource: snap.address || 'wallet', freeGasSol: snap.gas && isNum(snap.gas.sol) ? snap.gas.sol : null };
  }

  /** Cached (EQUITY_CACHE_MS) wallet-only baseline, plus its age. */
  async function walletEquitySnapshot() {
    if (!equityCache || now() - equityCache.atMs >= EQUITY_CACHE_MS) equityCache = { atMs: now(), ...(await readWalletEquity()) };
    return { ...equityCache, ageSec: Math.max(0, Math.round((now() - equityCache.atMs) / 1000)) };
  }

  /** Wallet-only baseline + open-position collateral +/- unrealized PnL. null when the baseline is unreadable. */
  function fullEquityUsd(baseEquityUsd, openPositions) {
    if (!isNum(baseEquityUsd)) return null;
    const adj = (Array.isArray(openPositions) ? openPositions : []).reduce((s, p) => s + (isNum(p.collateralUsd) ? p.collateralUsd : 0) + (isNum(p.unrealizedPnlUsd) ? p.unrealizedPnlUsd : 0), 0);
    return r2(baseEquityUsd + adj);
  }

  /** readPositions() rows -> the {symbol, sizeUsd, collateralUsd, unrealizedPnlUsd} shape riskPolicy wants. */
  const forPolicy = (positions) => (Array.isArray(positions) ? positions : []).map((p) => ({ symbol: p.symbol, sizeUsd: p.sizeUsd, collateralUsd: p.collateralUsd, unrealizedPnlUsd: p.unrealizedPnlUsd }));

  /** Env risk-policy config with prefsRisk overrides applied (bounded, tighten-only) and the executor's own env caps folded in for sizing. */
  function resolveRiskPolicy(cfg, prefsRisk) {
    return { ...applyRiskPrefs(readRiskPolicyConfig(env), prefsRisk), maxSizeCapUsd: cfg.caps.maxSizeUsd, maxLeverageCap: cfg.caps.maxLeverage };
  }
  ...
  /**
   * Post-close verify. A close is a decrease REQUEST that a keeper fills a few seconds
   * later (same as the open), so this polls every CLOSE_VERIFY_POLL_MS for up to
   * CLOSE_VERIFY_MAX_MS until the position is gone (full close) or smaller (partial).
   * (2026-09-26 live Close 50 %: the request landed, the keeper filled it ~5 s later, but a
   * single immediate read said size_unchanged and the close went unjournaled.)
   */
  async function verifyClose(o) {
    const startMs = now();
    let last = { ok: false, reason: 'size_unchanged' };
    for (;;) {
      try {
        const r = await readPositions();
        if (!r.ok) last = { ok: false, reason: 'positions_unavailable' };
        else {
          const hit = r.positions.find((p) => p && p.positionId === o.positionId);
          if (!o.sizeUsd) last = hit ? { ok: false, reason: 'position_still_open' } : { ok: true };
          else last = hit && !(hit.sizeUsd < o.positionSizeUsd) ? { ok: false, reason: 'size_unchanged' } : { ok: true };
        }
      } catch (err) {
        last = { ok: false, reason: `verify_${errName(err)}` };
      }
      if (last.ok || now() - startMs >= CLOSE_VERIFY_MAX_MS) return last;
      await sleep(CLOSE_VERIFY_POLL_MS);
    }
  }
  ...
  async function emergencyClose(j, signer, addr, meta, o, actionId) {
    ...
    for (;;) {
      ...
      try {
        ...
        if (sim.err) throw simFail('emergency close simulation failed', sim);
        ...
        if (attempt === 1) {
          ...
          await alert(`🛑 Naked position: emergency close FAILED, kill engaged. Retrying every ${EMERGENCY_CLOSE_RETRY_MS / 1000}s for up to ${Math.round(EMERGENCY_CLOSE_MAX_MS / 1000)} s.`, { positionIdHash: idHash(meta.positionId) });
        }
  ...
  async function preflight(intent, ctx = {}) {
    ...
    try {
      ...
      const dayPnl = await pnlWindow(store, now(), 1);
      const loss = dayPnl.ok ? { ok: true, lossUsd: r2(Math.max(0, -dayPnl.pnlUsd)) } : { ok: false, lossUsd: null };
      ...
      // Wallet-aware risk policy (T-8, lib/execution/riskPolicy.js): a layer on top of the
      // env caps above, read-only toward the chain. Its reasons refuse like any other gate;
      // a drawdown breach also engages the kill switch (arm clears it, the env kill still
      // wins) so it stays refused on every later call until the owner acknowledges it.
      const weekPnl = await pnlWindow(store, now(), 7);
      const wallet = await walletEquitySnapshot();
      const equityUsd = fullEquityUsd(wallet.baseEquityUsd, positions.ok ? positions.positions : []);
      const riskPolicyConfig = resolveRiskPolicy(cfg, ctx.riskPrefs);
      const riskPolicy = evaluateRiskPolicy({
        equityUsd,
        openPositions: forPolicy(positions.ok ? positions.positions : []),
        dailyPnlUsd: dayPnl.ok ? dayPnl.pnlUsd : null,
        weekPnlUsd: weekPnl.ok ? weekPnl.pnlUsd : null,
        freeGasSol: wallet.freeGasSol,
        intent: { symbol: derived.symbol, sizeUsd: i.sizeUsd, leverage: i.leverage, entry: i.entry, stop: i.stop },
        policy: riskPolicyConfig
      });
      reasons.push(...riskPolicy.reasons);
      if (riskPolicy.reasons.includes('daily_drawdown')) await setKill(store, { reason: 'risk_daily_drawdown' }, now()); // best effort
      if (riskPolicy.reasons.includes('weekly_drawdown')) await setKill(store, { reason: 'risk_weekly_drawdown' }, now()); // best effort
      ...
      const order = {
        ...
        equityUsd, equitySource: wallet.equitySource,
        riskUsd: riskPolicy.riskUsd, riskPct: riskPolicy.riskPct,
        exposurePctBefore: riskPolicy.exposurePctBefore, exposurePct: riskPolicy.exposurePct, symbolExposurePct: riskPolicy.symbolExposurePct,
        suggestedSizeUsd: riskPolicy.suggestedSizeUsd, suggestedLeverage: riskPolicy.suggestedLeverage
      };
      out.order = order;
      out.reasons = [...new Set(reasons)];
      out.ok = out.reasons.every((r) => r.startsWith('warn:'));
      await audit('preflight', {
        intent: auditIntent, ok: out.ok, reasons: out.reasons, quote: out.quote, order: out.ok ? order : null,
        risk: { ok: riskPolicy.ok, reasons: riskPolicy.reasons, equityUsd, riskPct: riskPolicy.riskPct, exposurePct: riskPolicy.exposurePct, drawdown: riskPolicy.drawdown }
      }, cfg.mode);
      return out;
    } catch (err) {
  ...
  async function executeOpen(order, ctx, nonce = null) {
    ...
    // F4: an optional ctx.onPhase(phase, meta) is called at the four milestones the
    // Telegram result card edits in place (submitted -> filled -> stops attached ->
    // verified). Best effort: a failing notify (e.g. a Telegram API error) never breaks
    // the trade.
    const onPhase = typeof ctx.onPhase === 'function' ? ctx.onPhase : null;
    const notify = async (phase, meta = {}) => { if (onPhase) { try { await onPhase(phase, meta); } catch { /* best effort */ } } };
    try {
      const j = await jupiter();
      ...
      await notify('submitted', { symbol: o.symbol, direction: o.direction, sizeUsd: o.sizeUsd, leverage: o.leverage });
      ...
      if (openSim.err) throw simFail('open simulation failed', openSim);
      ...
      try {
        ...
        if (stopsSim.err) throw simFail('stops simulation failed', stopsSim);
        ...
        await notify('stops_attached', { stop: o.stop, tp: o.tp1 });
      } catch (stopsErr) {
        // Also to the function log: the Blob audit can time out, and this is the one line
        // that says why the naked position had to be closed (2026-09-26 live run: lost).
        console.error(`[Execution] stops_attach failed actionId=${actionId} error=${JSON.stringify(errMsg(stopsErr))}`);
        await audit('error', { stage: 'stops_attach', actionId, positionIdHash: idHash(openBuilt.meta.positionId), error: errMsg(stopsErr) }, 'live');
        const closed = await emergencyClose(j, signer, addr, openBuilt.meta, o, actionId);
        return await recordActionResult(actionId, { ...closed, stopsError: errMsg(stopsErr) });
      }
  ...
  async function executeClose(order, ctx, actionIdSeed = null) {
    ...
    try {
      const j = await jupiter();
      // Slippage bound needs a USD reference: the live engine mark for the symbol, else the
      // position's mark, else its entry. Without it the wrapper threw "referencePrice (USD)
      // is required" on the first live Close 50 % (2026-09-26).
      const fill = await liveFill(o.symbol, ctx);
      const referencePrice = fill && pos(fill.price) ? fill.price : pos(p.markPrice) ? p.markPrice : pos(p.entryPrice) ? p.entryPrice : null;
      if (!pos(referencePrice)) throw new Error('close reference price unavailable');
      const r = await j.closePerpPosition(o.positionId, o.sizeUsd, { referencePrice });
  ...
  async function confirm(nonce, pin, ctx = {}) {
    try {
      ...
      // Every result names its action and the order's identity, so a caller that lost its
      // own ticket record (Blob lag, 2026-09-26: a full close rendered as an open card)
      // can still render the right card.
      const o = used.order;
      const tag = { action: o.action, symbol: o.symbol || null, direction: o.direction || null, positionId: o.positionId || null, sizeUsd: o.sizeUsd ?? null, positionSizeUsd: o.positionSizeUsd ?? null, stop: o.stop ?? null, tp: o.tp ?? null };
      if (o.action === 'open') return { ...tag, ...(await executeOpen(o, ctx, String(nonce))) };
      if (o.action === 'close') return { ...tag, ...(await executeClose(o, ctx, String(nonce))) };
      if (o.action === 'update') return { ...tag, ...(await executeUpdate(o, ctx, String(nonce))) };
  ...
  /**
   * Most recent execution open/adjust record for positionIdHash (last OPEN_LOOKUP_DAYS
   * days): {stop, tp} or null. The on-chain Position account itself carries no SL/TP, and
   * a pending trigger-request PDA cannot be enumerated from the position alone (its seed
   * includes a random counter), so the journal -- what stop/TP the executor last set --
   * is the source of truth for "/positions" stops display (F4).
   */
  async function latestStops(positionIdHash) {
    if (!positionIdHash) return null;
    try {
      for (let d = 0; d < OPEN_LOOKUP_DAYS; d++) {
        const day = new Date(now() - d * 86_400_000).toISOString().slice(0, 10);
        const b = await readBlob(store.get, journalDayPath(day));
        const hit = parseJsonlObjects(b && b.text).reverse().find((r) => (r.kind === 'open' || r.kind === 'adjust') && r.source === 'execution' && r.execRef && r.execRef.positionIdHash === positionIdHash);
        if (hit) return { stop: isNum(hit.stop) ? hit.stop : null, tp: isNum(hit.tp1) ? hit.tp1 : null };
      }
    } catch { /* best effort */ }
    return null;
  }
  ...
  async function listPositions() {
    const r = await readPositions();
    if (!r.ok) return { ok: r.ok, positions: r.positions, error: r.error };
    const positions = await Promise.all(r.positions.map(async (p) => {
      const stops = await latestStops(idHash(p.positionId));
      return stops ? { ...p, stop: stops.stop, tp: stops.tp } : p;
    }));
    return { ok: true, positions, error: null };
  }

  /** @param {Object} [prefsRisk] - state.prefs.risk overrides (T-8 /risk), bounded as usual. */
  async function status(prefsRisk) {
    try {
      const cfg = readExecutionConfig(env);
      const kill = killStatus(cfg, await readKillState(store), now());
      const dayPnl = await pnlWindow(store, now(), 1);
      const weekPnl = await pnlWindow(store, now(), 7);
      const loss = dayPnl.ok ? r2(Math.max(0, -dayPnl.pnlUsd)) : null;
      let openCount = null;
      let openPositions = [];
      if (cfg.enabled) {
        const p = await readPositions();
        openCount = p.ok ? p.positions.length : null;
        openPositions = p.ok ? p.positions : [];
      }
      let walletMarginUsd = null;
      if (deps.getMarginUsd) { try { walletMarginUsd = r2(await deps.getMarginUsd()); } catch { walletMarginUsd = null; } }
      const wallet = await walletEquitySnapshot();
      const equityUsd = fullEquityUsd(wallet.baseEquityUsd, openPositions);
      const riskPolicyConfig = resolveRiskPolicy(cfg, prefsRisk);
      const riskSnap = evaluateRiskPolicy({
        equityUsd, openPositions: forPolicy(openPositions),
        dailyPnlUsd: dayPnl.ok ? dayPnl.pnlUsd : null, weekPnlUsd: weekPnl.ok ? weekPnl.pnlUsd : null,
        freeGasSol: wallet.freeGasSol, policy: riskPolicyConfig
      });
      return {
        ...
        caps: cfg.caps, dailyLossUsd: loss, openCount, walletMarginUsd,
        liveCapabilities: { ...caps },
        risk: {
          equityUsd, equitySource: wallet.equitySource, equityAgeSec: wallet.ageSec,
          exposurePct: riskSnap.exposurePct, drawdown: riskSnap.drawdown, reasons: riskSnap.reasons,
          policy: {
            pctPerTrade: riskPolicyConfig.pctPerTrade, maxExposurePct: riskPolicyConfig.maxExposurePct,
            maxPerSymbolPct: riskPolicyConfig.maxPerSymbolPct, dailyDrawdownPct: riskPolicyConfig.dailyDrawdownPct,
            weeklyDrawdownPct: riskPolicyConfig.weeklyDrawdownPct, minFreeGasSol: riskPolicyConfig.minFreeGasSol
          }
        }
      };
    } catch (err) {
  ...
  /**
   * The highest value a `/risk <key> VALUE` override may set right now (env default,
   * capped at RISK_PCT_PER_TRADE_MAX for pctPerTrade). Pure, sync -- exposed so the
   * webhook (which never imports lib/execution/riskPolicy.js directly; this executor is
   * its one sanctioned door into lib/execution) can validate an override before storing it.
   */
  const riskPrefBound = (key) => riskOverrideBound(key, readRiskPolicyConfig(env));

  return { preflight, createTicket, cancelTicket, confirm, closePosition, updateStops, listPositions, status, prepareClose, prepareUpdate, kill, arm, riskPrefBound };
}
...
export const status = (prefsRisk) => def().status(prefsRisk);
...
export const riskPrefBound = (key) => def().riskPrefBound(key);
