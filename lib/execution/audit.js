 ...
 * SOLANA_RPC_URL, BLOB_READ_WRITE_TOKEN are replaced wherever they appear. `txSignature`
 * is the one base58 field kept (a public transaction id). appendAudit never throws.
 */
...
const KEEP_KEYS = new Set(['txSignature']);
...
export function redact(value, env = process.env, secrets = secretValues(env), depth = 0) {
  ...
  if (value && typeof value === 'object') {
    ...
    for (const [k, v] of Object.entries(value)) {
      if (KEEP_KEYS.has(k) && typeof v === 'string' && /^[1-9A-HJ-NP-Za-km-z]{32,90}$/.test(v) && !secrets.includes(v)) { out[k] = v; continue; }
