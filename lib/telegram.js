...
export const COMMANDS = Object.freeze(['start', 'help', 'menu', 'signals', 'why', 'flags', 'wallet', 'journal', 'status', 'chart', 'charts', 'log', 'testalert', 'alerts', 'tracking', 'positions', 'market',
  'order', 'confirm', 'stops', 'exec', 'kill', 'arm', 'mode', 'risk']);
...
export function normalizePrefs(raw) {
  ...
  return { level, quiet, alertTimeframes, risk: normalizeRiskPrefs(p.risk) };
}
...
export function formatExecStatus(status, env = {}) {
  ...
  return joinSections([
    ...
    '/kill stops execution now · /arm PIN clears /kill (an env kill stays) · /risk shows the policy · mode is env-only'
  ]);
}
