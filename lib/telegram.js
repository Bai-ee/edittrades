...
function gateFor(rec, c) {
  ...
  const blockers = reasons.filter(isGateBlocker);
  ...
  const text = blockers.map((code) => {
    ...
    const need = risk ? brk + (c.direction === 'short' ? -1 : 1) * MIN_GROSS_RR * risk : null;
    return `R ${m[1]} under ${MIN_GROSS_RR} floor${need !== null ? `; needs TP beyond ${fmtLevel(need)}` : ''}`;
  }).join('; ');
...
function contextLines(rec, c, mark) {
  ...
  if (cl) {
    ...
    for (const code of [...reasons.filter(isGateBlocker), ...reasons.filter((x) => !isGateBlocker(x))]) lines.push(escapeHtml(reasonPhrase(code, 'oppose')));
