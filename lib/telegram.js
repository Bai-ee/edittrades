...
const LEVEL_KINDS = {
  good: ['GOOD', 'GOOD_ENDED'],
  setup: ['GOOD', 'GOOD_ENDED', 'SETUP'],
  watch: ['GOOD', 'GOOD_ENDED', 'SETUP', 'WATCH', 'TRIGGERING']
};
...
/** Charts: BTC/ETH/SOL x 1m/3m/5m/15m/1h, callback_data `chart:BTC:1m`. */
export function chartsKeyboard() {
  return { inline_keyboard: ['BTC', 'ETH', 'SOL'].map((sym) => CHART_GRID_TIMEFRAMES.map((tf) => ({ text: `${sym} ${tf}`, callback_data: `chart:${sym}:${tf}` }))) };
}
...
export function parseState(text) {
  ...
  return {
    ...
    watch: {
      ids: isObj(raw.watch) && Array.isArray(raw.watch.ids) ? raw.watch.ids.filter((e) => isObj(e) && typeof e.id === 'string').slice(-WATCH_RECENT_IDS) : [],
      lastAt: isObj(raw.watch) && isObj(raw.watch.lastAt) ? raw.watch.lastAt : {}
    },
 ...
 * @param {number} nowMs
 * GOOD and SETUP alerts carry `replyMarkup` (Why / Chart / Took it / Skipped) and store the
...
export function diffAlerts(prevState, payload, nowMs) {
  ...
  for (const k of Object.keys(syms).sort()) {
    ...
    if (dataOk && next.setupId && !next.setupIds.includes(next.setupId)) {
      ...
      if (allow('SETUP')) {
        alerts.push({ kind: 'SETUP', symbol: k, text: formatSetupAlert(k, rec.setup), replyMarkup: { inline_keyboard: [tradeButtonRow(k, rec.setup.timeframe, next.setupId)] } });
...
export default {
  ...
  formatAlertPrefs, fmtQuiet, parseAlertsArgs, parseMenuLabel, menuKeyboard, chartsKeyboard, alertsKeyboard, shortRef,
