...
export function diffTracked(state, payload, nowMs) {
  ...
  for (const t of Array.isArray(state.tracked) ? state.tracked : []) {
      ...
      if (!t.hit && px !== null) {
        ...
        if (stopHit || tpHit) {
          ...
          if (t.took) {
            ...
            push(t, joinSections([h, levels(px, src, r), `Journal it: ${kind === 'tp1' ? 'Closed here, Partial or Still in' : 'Closed here or Still in'}.`]), hitKeyboard(t.ref, kind), 'TRACK', null, kind);
            keep.push({ ...t, hit });
          } else push(t, joinSections([h, levels(px, src, r), 'Tracking ended (you did not tap Took it).']), null, 'TRACK', null, kind);
          continue;
        }
    ...
    if (through || (c && c.state === 'failed')) { ended(t, 'VOID', through ? `Closed through void ${lvl(inv)}.` : 'The flag failed.'); continue; }
    if (!c) { ended(t, 'GONE', 'The flag is no longer detected.'); continue; }
    if (c.state === 'expired') { ended(t, 'EXPIRED', 'No breakout in time.'); continue; }
    const v = liveView(t.symbol, s, t.candidateId, payload);
    const next = { ...t, lastState: c.state, setupSeen: t.setupSeen || Boolean(v.setup), breakoutLevel: isNum(c.breakoutLevel) ? c.breakoutLevel : t.breakoutLevel, invalidation: inv };
    ...
    if (v.plan && v.plan.status === 'ready') {
      ...
      push(t, alertMessage('TRACK · GET IN NOW', t.symbol, c, { ...opts, verdictKind: 'BREAKOUT' }), kb, 'TRACK', [formatPlanCard(v)], 'get_in_now');
      alerts[alerts.length - 1].chart = { symbol: t.symbol, timeframe: t.timeframe || v.plan.timeframe };
    } else if (v.setup && !t.setupSeen) {
      push(t, alertMessage('TRACK · SETUP', t.symbol, c, { ...opts, verdictKind: 'SETUP' }), kb, 'TRACK', null, 'setup');
    } else if (c.state !== t.lastState) {
      push(t, alertMessage(`TRACK · ${escapeHtml(String(c.state).toUpperCase())}`, t.symbol, c, { ...opts, verdictKind: kindForState(c.state), verdict: verdictFor(v, { levels: false }) }), kb, 'TRACK', null, `state:${c.state}`);
    }
    keep.push(next);
  }
  state.tracked = keep;
 ...
 * on: its own plan with status ready and, when the recommendation carries a readiness
 * call, that call is GET IN NOW.
 */
export function isOpenReady(v) {
  if (!isObj(v) || v.source !== 'live' || !isObj(v.plan) || v.plan.status !== 'ready') return false;
  const call = isObj(v.rec) && isObj(v.rec.action) ? v.rec.action.call : null;
  return typeof call !== 'string' || call === 'GET IN NOW';
}
...
export function orderIntentFromPlan(v, caps = {}) {
  if (!isOpenReady(v)) return { error: 'the plan is not ready (Open needs GET IN NOW)' };
  const p = v.plan;
  ...
  const intent = {
    ...
    planId: p.planId || null, candidateId: v.candidateId, source: 'telegram'
  };
...
export function formatResultCard(result, t, { tracking = null } = {}) {
  ...
  return joinSections([
    execHeader(dry ? '🧪' : '✅', dry ? 'DRY RUN OK' : 'FILLED', tk.symbol || pos.symbol || '?', tk.timeframe || null, tk.direction || pos.direction || pos.side),
    codeBlock([
      ['price', lvl(price)],
      ['size', `${fmtUsd(size)}${isNum(lev) ? ` · ${lev}x` : ''}`],
      ['position', id ? shortId(id) : (dry ? 'n/a (dry run)' : 'n/a')],
      ['SL', lvl(firstNum(pos.stop, pos.stopLoss, o.stop, tk.stop))],
      ['TP1', lvl(firstNum(pos.tp1, pos.takeProfit, pos.tp, o.tp1, tk.tp1))]
    ]),
    [dry ? `dry-run id ${escapeHtml(shortId(r.dryRunId || 'n/a'))} · journaled as a note` : `tx ${escapeHtml(shortId(r.txSignature || 'n/a'))}`,
