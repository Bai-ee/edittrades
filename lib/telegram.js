...
export const MENU_ROWS = Object.freeze([['Signals', 'Flags', 'Market'], ['Why BTC', 'Why ETH', 'Why SOL'], ['Charts', 'Wallet', 'Positions'], ['Journal', 'Status', 'Alerts', 'Tracking']]);
...
const LABEL_COMMANDS = {
  ...
  tracking: ['tracking'], positions: ['positions'], market: ['market']
};
...
export function formatHelp() {
  return [
    ...
    '/menu — show the button keyboard (Signals, Flags, Market, Why, Charts, Wallet, Positions, Journal, Status, Alerts, Tracking)',
...
export function diffTracked(state, payload, nowMs) {
  ...
  const alerts = [];
  const keep = [];
  const push = (t, text, replyMarkup = null, kind = 'TRACK', more = null) => alerts.push({ kind, symbol: t.symbol, candidateId: t.candidateId, ref: t.ref, text, ...(replyMarkup ? { replyMarkup } : {}), ...(more ? { more } : {}) });
  const head = (t, dot, kind) => msgHeader(dot, t.symbol, t.timeframe, t.direction, kind);
  const ended = (t, kind, detail) => push(t, joinSections([head(t, '🔴', `TRACK · ${kind}`), detail, `${bold('STAND DOWN')} — tracking ended.`]));
  for (const t of Array.isArray(state.tracked) ? state.tracked : []) {
    if (!isObj(t)) continue;
    if (!(nowMs - Date.parse(t.since) < TRACK_TTL_MS)) {
      push(t, joinSections([head(t, '⚪', 'TRACK ENDED'), '6 h limit reached. Tap Track again to keep following it.']));
      continue;
    }
      ...
      if (!t.hit && px !== null) {
        ...
        if (stopHit || tpHit) {
          ...
          if (t.took) {
            ...
            push(t, joinSections([h, levels(px, src, r), `Journal it: ${kind === 'tp1' ? 'Closed here, Partial or Still in' : 'Closed here or Still in'}.`]), hitKeyboard(t.ref, kind));
            keep.push({ ...t, hit });
          } else push(t, joinSections([h, levels(px, src, r), 'Tracking ended (you did not tap Took it).']));
          continue;
        }
        ...
        ]), hitKeyboard(t.ref, t.hit.kind), 'NUDGE');
    ...
    if (v.plan && v.plan.status === 'ready') {
      ...
      push(t, alertMessage('TRACK · GET IN NOW', t.symbol, c, { ...opts, verdictKind: 'BREAKOUT' }), kb, 'TRACK', [formatPlanCard(v)]);
      alerts[alerts.length - 1].chart = { symbol: t.symbol, timeframe: t.timeframe || v.plan.timeframe };
    } else if (v.setup && !t.setupSeen) {
      push(t, alertMessage('TRACK · SETUP', t.symbol, c, { ...opts, verdictKind: 'SETUP' }), kb);
    } else if (c.state !== t.lastState) {
      push(t, alertMessage(`TRACK · ${escapeHtml(String(c.state).toUpperCase())}`, t.symbol, c, { ...opts, verdictKind: kindForState(c.state), verdict: verdictFor(v, { levels: false }) }), kb);
    }
    keep.push(next);
  }
  state.tracked = keep;
