...
/** Focus mode (owner pref in state.prefs.focus, T-7): 'auto' mutes alerts unrelated to an
 * open position; 'off' sends everything. Default 'auto'. */
export const FOCUS_MODES = Object.freeze(['auto', 'off']);
export const DEFAULT_FOCUS_MODE = 'auto';
/** api/telegram-cron.js live-position snapshot (state.livePositions): reused while younger than this. */
export const LIVE_POSITIONS_CACHE_MS = 60_000;
/** Alert kinds that always bypass focus mode (health / system, no position tie). */
export const FOCUS_ALWAYS_KINDS = Object.freeze(['MARK', 'DATA', 'DATA_OK']);
const LEVEL_KINDS = {
...
export const COMMANDS = Object.freeze(['start', 'help', 'menu', 'signals', 'why', 'flags', 'wallet', 'journal', 'status', 'chart', 'charts', 'log', 'testalert', 'alerts', 'tracking', 'positions', 'market',
  'order', 'confirm', 'stops', 'exec', 'kill', 'arm', 'mode', 'risk', 'focus']);
/** Persistent reply keyboard rows; each label maps to a command (parseMenuLabel). */
export const MENU_ROWS = Object.freeze([['Signals', 'Flags', 'Market'], ['Why BTC', 'Why ETH', 'Why SOL'], ['Charts', 'Wallet', 'Positions', 'Exec'], ['Journal', 'Status', 'Alerts', 'Tracking'], ['Focus']]);
...
export function parseAlertsArgs(args) {
  ...
  const usage = 'Usage: /alerts · /alerts good|setup|watch · /alerts quiet [HH-HH|off] · /alerts tf 5m|3m,5m|all · /alerts focus auto|off';
  ...
  if (a[0] === 'focus') {
    return a.length === 2 && FOCUS_MODES.includes(a[1])
      ? { action: 'focus', focus: a[1] }
      : { action: 'error', message: 'Usage: /alerts focus auto|off (auto mutes alerts unrelated to an open position; off sends everything)' };
  }
...
export const RISK_USAGE = 'Usage: /risk · /risk pct|exposure|symbolexposure|dailydd|weeklydd|gas VALUE · /risk reset';
/** `/risk` short arg name -> the RISK_PREF_KEYS key it writes. */
const RISK_ARG_KEYS = Object.freeze({ pct: 'pctPerTrade', exposure: 'maxExposurePct', symbolexposure: 'maxPerSymbolPct', dailydd: 'dailyDrawdownPct', weeklydd: 'weeklyDrawdownPct', gas: 'minFreeGasSol' });

/**
 * `/risk` arguments -> an action. [] -> show; reset -> clear all overrides; KEY VALUE ->
 * set (the executor bounds VALUE against env caps and the 2% ceiling, not this parser).
 */
export function parseRiskArgs(args) {
  const a = (Array.isArray(args) ? args : []).map((x) => String(x));
  if (!a.length) return { action: 'show' };
  if (a.length === 1 && a[0].toLowerCase() === 'reset') return { action: 'reset' };
  if (a.length !== 2) return { action: 'error', message: RISK_USAGE };
  const key = RISK_ARG_KEYS[a[0].toLowerCase()];
  if (!key) return { action: 'error', message: RISK_USAGE };
  const value = Number(a[1]);
  if (!Number.isFinite(value) || value <= 0) return { action: 'error', message: `${a[0]} must be a positive number.` };
...
const LABEL_COMMANDS = {
  ...
  tracking: ['tracking'], positions: ['positions'], market: ['market'], exec: ['exec'], focus: ['focus']
};
...
export function alertsKeyboard() {
  return {
    inline_keyboard: [
      ...
      [{ text: 'Focus auto', callback_data: 'alerts:focus:auto' }, { text: 'Focus off', callback_data: 'alerts:focus:off' }]
    ]
  };
}
...
export function parseCallbackData(data) {
  ...
  if (d === 'alerts:focus:auto') return cmd('alerts', ['focus', 'auto']);
  if (d === 'alerts:focus:off') return cmd('alerts', ['focus', 'off']);
...
/**
 * Alert clarity (schema 1.27.0) for candidate `c`: flagRecommendation.clarity when it was
 * built for this candidate, else null (the record names one subject candidate per symbol).
 */
function clarityFor(rec, c) {
  const cl = isObj(rec) && isObj(rec.clarity) ? rec.clarity : null;
  return cl && isObj(c) && c.candidateId && cl.candidateId === c.candidateId ? cl : null;
}

/** The plan's gross R floor as the record publishes it (clarity.gate.minRR, schema 1.27.0); MIN_GROSS_RR when absent. */
function minRRFor(rec) {
  const g = isObj(rec) && isObj(rec.clarity) && isObj(rec.clarity.gate) ? rec.clarity.gate : null;
  return g && isNum(g.minRR) ? g.minRR : MIN_GROSS_RR;
}

/** A qual code that stops the flag passing the plan gates (mirror of flagRecommendation buildClarity). */
function isGateBlocker(code, minRR = MIN_GROSS_RR) {
  if (/^room:blocked/.test(code) || code === 'chase') return true;
  const m = String(code).match(/^rr:([\d.]+)$/);
  return Boolean(m) && Number(m[1]) < minRR;
}

/**
 * The gate for candidate `c`: flagRecommendation.clarity.gate when the record's subject is
 * this candidate, else the same read from the candidate's own `qual.reasons` (a WATCH or
 * TRIGGERING alert can name a flag other than the record's subject). Null when unknown.
 */
function gateFor(rec, c) {
  const cl = clarityFor(rec, c);
  if (cl && isObj(cl.gate)) return cl.gate;
  const reasons = isObj(c) && isObj(c.qual) && Array.isArray(c.qual.reasons) ? c.qual.reasons : null;
  if (!reasons) return null;
  const minRR = minRRFor(rec);
  const blockers = reasons.filter((x) => isGateBlocker(x, minRR));
  if (!blockers.length) return { passable: true, blockers, text: null };
  const brk = isNum(c.breakoutLevel) ? c.breakoutLevel : null;
  const risk = brk !== null && isNum(c.invalidation) ? Math.abs(brk - c.invalidation) : null;
  const text = blockers.map((code) => {
    const m = code.match(/^rr:([\d.]+)$/);
    if (!m) return reasonPhrase(code, 'oppose');
    const need = risk ? brk + (c.direction === 'short' ? -1 : 1) * minRR * risk : null;
    return `R ${m[1]} under ${minRR} floor${need !== null ? `; needs TP beyond ${fmtLevel(need)}` : ''}`;
  }).join('; ');
  return { passable: false, blockers, text };
}

/**
 * Context lines for an alert (schema 1.27.0): flagRecommendation.clarity.context, Kill if
 * and Other side when the record's subject is this candidate; else the same lines read
 * from the candidate's qual, the record's top-down code and divergence codes (no counts,
 * no other side). Then the mark. Never part of the alert signature.
 */
function contextLines(rec, c, mark) {
  const direction = c.direction;
  const cl = clarityFor(rec, c);
  const lines = [];
  if (cl) {
    for (const x of Array.isArray(cl.context) ? cl.context : []) lines.push(escapeHtml(x));
    if (isObj(cl.killIf) && cl.killIf.text) lines.push(`Kill if: ${escapeHtml(cl.killIf.text)}`);
    if (isObj(cl.otherSide) && cl.otherSide.text) lines.push(`Other side: ${escapeHtml(cl.otherSide.text)}`);
  } else {
    const reasons = isObj(c.qual) && Array.isArray(c.qual.reasons) ? c.qual.reasons : [];
    const minRR = minRRFor(rec);
    // Blockers already sit in the verdict line (gateFor); only the rest go here.
    for (const code of reasons.filter((x) => !isGateBlocker(x, minRR))) lines.push(escapeHtml(reasonPhrase(code, 'oppose')));
    ...
    if (td && td[2] === '2') lines.push('Top-down: split 2/4');
    else if (td && ((td[1] === 'bull' && direction === 'short') || (td[1] === 'bear' && direction === 'long'))) lines.push(`Counter-trend: top-down ${td[1]} ${td[2]}/4, against the ${escapeHtml(direction)}`);
    else if (td) lines.push(`Top-down: ${escapeHtml(td[1])} ${td[2]}/4`);
    const agrees = codes.includes('divergence_agrees');
    const against = codes.includes('divergence_conflicts');
    if (agrees || against) lines.push(`Divergence: ${[agrees ? 'agrees' : null, against ? 'against' : null].filter(Boolean).join(', ')}`);
    ...
    if (isNum(c.breakoutLevel)) lines.push(`Kill if: ${holdWord ? `EMA21 ${holdWord} already — ` : ''}close back ${direction === 'short' ? 'above' : 'below'} ${fmtLevel(c.breakoutLevel)} after a probe = defended, stand down`);
  }
  if (isObj(mark)) {
...
export function alertMessage(kind, symbol, c, { rec = null, plan = null, asOf = null, mark = null, verdictKind = kind, verdict = null } = {}) {
  ...
  return joinSections([head, levels, v, planSec, contextLines(rec, c, mark)]);
}
 ...
 *   WATCH / TRIGGERING whose gate is not passable (schema 1.27.0, flagRecommendation
 *     clarity.gate or the candidate's own qual): only rr blockers -> WAIT (eta) — gate
 *     text (a later TP could fix it); room/chase -> STAND DOWN — gate text
...
export function alertVerdict(kind, c, { plan = null, rec = null, asOf = null, levels = true } = {}) {
  ...
  if (kind === 'WATCH' || kind === 'TRIGGERING') {
    const gate = gateFor(rec, c);
    if (gate && gate.passable === false) {
      const text = escapeHtml(gate.text || 'flag cannot pass the plan gates');
      const rrOnly = Array.isArray(gate.blockers) && gate.blockers.length > 0 && gate.blockers.every((b) => String(b).startsWith('rr:'));
      return rrOnly ? `${bold('WAIT', etaToNextClose(asOf, tf))} — ${text}` : `${bold('STAND DOWN')} — ${text}`;
    }
  }
...
export function formatStatus(payload, state, nowMs, health = null) {
  ...
  rows.push(['Focus', formatFocusState(st.prefs, st.livePositions)]);
...
/**
 * "auto (SOL open)" / "auto (no position)" / "off" — the owner's focus pref plus, when
 * auto, whether a live position is currently open (state.livePositions, T-7).
 */
export function formatFocusState(prefs, livePositions) {
  const focus = normalizePrefs(prefs).focus;
  if (focus === 'off') return 'off';
  const syms = isObj(livePositions) && Array.isArray(livePositions.symbols) ? livePositions.symbols.filter((s) => typeof s === 'string') : [];
  return syms.length ? `auto (${syms.join('+')} open)` : 'auto (no position)';
}
...
export function formatAlertPrefs(prefs) {
  ...
  return [
    ...
    `Focus: <b>${p.focus}</b> — ${p.focus === 'off' ? 'every alert sends regardless of open positions.' : 'while a position is open, only that symbol\'s alerts, its tracking and health/data alerts send.'}`,
    'Change: /alerts good|setup|watch · /alerts quiet HH-HH · /alerts quiet off · /alerts tf 5m|3m,5m|all · /alerts focus auto|off'
  ].join('\n');
}
...
export function formatHelp() {
  return [
    ...
    '/menu — show the button keyboard (Signals, Flags, Market, Why, Charts, Wallet, Positions, Exec, Journal, Status, Alerts, Tracking, Focus)',
    '/charts — chart picker buttons',
    '/alerts — show alert level, quiet hours, timeframes and focus mode',
    ...
    '/alerts focus auto|off (or the Focus button, which toggles) — auto (default) mutes every alert unrelated to an open position until it closes, except tracking and health/data alerts; off sends everything',
    ...
    'Open — order ticket from the alert\'s own levels: "Open @ plan" on a ready GOOD/GET IN NOW plan, "Open (early)" on a SETUP, BREAKOUT or tracked setup that has entry/stop/TP1 but is not yet ready; size and leverage = engine suggestion capped by your caps; Confirm, then reply /confirm &lt;nonce&gt; &lt;PIN&gt; within 60 s (the message is deleted after use)',
    ...
    '/exec — mode, kill switch, caps, focus, loss today, open count, margin · /kill — stop all execution now (no PIN) · /arm &lt;PIN&gt; — clear the /kill flag · /mode — dry or live (env-only)',
    '/risk — wallet-aware risk policy: equity, exposure, drawdown, the per-trade/exposure/drawdown/gas caps · /risk pct|exposure|symbolexposure|dailydd|weeklydd|gas VALUE tightens one (never above the env default or 2% per trade) · /risk reset'
  ].join('\n');
}
...
/** A percent as the risk lines print it ("0.5%"; strips a trailing ".0"). */
const fmtPct = (v) => (isNum(v) ? `${(Math.round(v * 10) / 10).toFixed(1).replace(/\.0$/, '')}%` : 'n/a');
...
/**
 * The risk-policy override keys a `/risk` command may write to `state.prefs.risk` (T-8).
 * Kept in sync by hand with lib/execution/riskPolicy.js's RISK_ENV -- telegram.js imports
 * nothing from lib/execution on purpose (see the module header), so this is a small,
 * duplicated, pure allow-list rather than a shared import. The executor is the only place
 * that turns these into a bound (env caps, the absolute 2% ceiling): this file only keeps
 * the stored shape clean (known keys, positive finite numbers).
 */
export const RISK_PREF_KEYS = Object.freeze(['pctPerTrade', 'maxExposurePct', 'maxPerSymbolPct', 'dailyDrawdownPct', 'weeklyDrawdownPct', 'minFreeGasSol']);

/** An arbitrary object -> only the known risk-pref keys, positive finite numbers. Never throws. */
export function normalizeRiskPrefs(raw) {
  const p = isObj(raw) ? raw : {};
  const out = {};
  for (const key of RISK_PREF_KEYS) { if (typeof p[key] === 'number' && Number.isFinite(p[key]) && p[key] > 0) out[key] = p[key]; }
  return out;
}

/**
 * Stored prefs -> {level, quiet, alertTimeframes, risk}. Unknown level -> 'setup'. quiet:
 * missing -> the default 01-05 Chicago; null -> off (the owner turned it off); malformed
 * -> default. alertTimeframes: missing or malformed -> ['3m','5m']; null -> all. risk:
 * sanitized to known keys / positive numbers only (T-8 /risk overrides); {} = no override.
 */
export function normalizePrefs(raw) {
  ...
  const focus = FOCUS_MODES.includes(p.focus) ? p.focus : DEFAULT_FOCUS_MODE;
  return { level, quiet, alertTimeframes, focus, risk: normalizeRiskPrefs(p.risk) };
}
...
export function applyPrefsChange(text, change) {
  ...
  if (change && FOCUS_MODES.includes(change.focus)) prefs.focus = change.focus;
  state.prefs = normalizePrefs(prefs);
  return `${JSON.stringify(state, null, 2)}\n`;
}

/**
 * Apply a `/risk` change to stored state text (T-8); returns the new state text. The
 * caller (api/telegram-webhook.js) has already bound `value` against env caps and the 2%
 * ceiling (lib/execution/riskPolicy.js) before calling this -- this function only writes
 * the already-accepted value (or clears every override on `reset`).
 * @param {string|null} text - current telegram/state.json
 * @param {{action:'set', key:string, value:number}|{action:'reset'}} change
 */
export function applyRiskPrefsChange(text, change) {
  const state = parseState(text);
  const risk = { ...state.prefs.risk };
  if (change && change.action === 'reset') {
    for (const key of RISK_PREF_KEYS) delete risk[key];
  } else if (change && change.action === 'set' && RISK_PREF_KEYS.includes(change.key)) {
    risk[change.key] = change.value;
  }
  state.prefs = normalizePrefs({ ...state.prefs, risk });
  return `${JSON.stringify(state, null, 2)}\n`;
}
...
/** Stored live-position cache (T-7, api/telegram-cron.js) -> {at, symbols, positionIds} or null when absent/malformed. */
export function normalizeLivePositions(raw) {
  if (!isObj(raw) || !isoOrNull(raw.at) || !Array.isArray(raw.symbols) || !Array.isArray(raw.positionIds)) return null;
  return { at: raw.at, symbols: strList(raw.symbols), positionIds: strList(raw.positionIds) };
}
...
function migrateRaw(raw) {
  ...
  return {
    ...
    tracked: (Array.isArray(raw.tracked) ? raw.tracked : [])
      ...
      .slice(-TRACK_MAX),
    livePositions: normalizeLivePositions(raw.livePositions)
  };
}
 ...
 */
/**
 * Focus mode (T-7, api/telegram-cron.js): true when `alert` may send while a position is
 * open on `openSymbols` -- its own symbol matches an open position (covers TRACK, GOOD,
 * SETUP, BREAKOUT, WATCH, GOOD_ENDED alike), or it is a health/system kind that always
 * bypasses focus (MARK, DATA, DATA_OK; symbol-less DATA trivially cannot match anyway).
 */
export function focusRelated(alert, openSymbols) {
  if (!isObj(alert)) return false;
  if (FOCUS_ALWAYS_KINDS.includes(alert.kind)) return true;
  const syms = openSymbols instanceof Set ? openSymbols : new Set(Array.isArray(openSymbols) ? openSymbols : []);
  return typeof alert.symbol === 'string' && syms.has(alert.symbol);
}
...
/** The markup with an `Open` row (open:<ref>) on top; `ready` picks the label (T-7). */
export function withOpenButton(markup, candidateId, ready = true) {
  const rows = isObj(markup) && Array.isArray(markup.inline_keyboard) ? markup.inline_keyboard : [];
  return { inline_keyboard: [[{ text: ready ? 'Open @ plan' : 'Open (early)', callback_data: `open:${shortRef(candidateId)}` }], ...rows] };
}

/** Alert kinds/events an Open button may attach to (T-7): a ready plan, or an early flag
 * with entry/stop/TP1 on file. Never WATCH/TRIGGERING (no levels worth trading yet). */
const OPEN_ELIGIBLE_KINDS = new Set(['GOOD', 'SETUP', 'BREAKOUT']);
const OPEN_ELIGIBLE_TRACK_EVENTS = new Set(['get_in_now', 'setup']);
export function openEligible(alert) {
  if (!isObj(alert)) return false;
  if (OPEN_ELIGIBLE_KINDS.has(alert.kind)) return true;
  return alert.kind === 'TRACK' && OPEN_ELIGIBLE_TRACK_EVENTS.has(alert.event);
}

/**
 * Entry/stop/TP1 (+direction) an Open tap would use for live view `v` (resolveRef/liveView),
 * or null when there is nothing tradeable yet. Live only — a snapshot of a candidate no
 * longer in the payload (possibly failed/expired since) is never a source for new levels.
 * A ready plan (isOpenReady) uses the plan; otherwise the candidate's own breakout /
 * invalidation / measured-target levels (SETUP uses the setup object when present).
 */
export function candidateLevels(v) {
  if (!isObj(v) || v.source !== 'live') return null;
  if (isOpenReady(v)) {
    const p = v.plan;
    return { entry: p.entry, stop: p.stop, tp1: p.tp1, tp2: isNum(p.tp2) ? p.tp2 : null, direction: p.direction || (isObj(v.candidate) ? v.candidate.direction : null) || null, ready: true };
  }
  // This candidate IS the symbol's own plan but not (yet) ready (e.g. BE READY, one more
  // confirmation pending): that is the pre-ready GOOD path, not an early SETUP/BREAKOUT
  // opportunity, so no Open at all -- never offer a trade the plan itself is still holding
  // back on. A plan.status of 'rejected' is the exception: that plan is dead, and a
  // chase-rejected candidate's SETUP reinterpretation (below) is what remains tradeable.
  if (isObj(v.plan) && v.plan.status !== 'rejected') return null;
  const c = isObj(v.candidate) ? v.candidate : {};
  const setup = isObj(v.setup) ? v.setup : null;
  // Never WATCH/TRIGGERING/forming: an early Open needs a SETUP-tier candidate (setup !=
  // null, chase-rejected confirmed flag per PLAN_TELEGRAM.md "Chase SETUP") or a confirmed
  // one (BREAKOUT); explicit on c.state so this never rides on incidental field presence.
  if (!setup && c.state !== 'confirmed') return null;
  const src = setup || c;
  const entry = firstNum(src.breakoutLevel, src.entry);
  const stop = firstNum(src.invalidation, src.stop);
  const tp1 = firstNum(src.measuredTarget, src.tp1);
  if (![entry, stop, tp1].every(isNum)) return null;
  const direction = (setup && setup.direction) || c.direction || null;
  if (!direction) return null;
  return { entry, stop, tp1, tp2: null, direction, ready: false };
}

/** "GOOD" / "SETUP" / "BREAKOUT" / "FLAG" — the source an Open tap resolved against, for the ticket card's `from` line. */
export function openSourceKind(v) {
  if (isOpenReady(v)) return 'GOOD';
  if (isObj(v) && isObj(v.setup)) return 'SETUP';
  if (isObj(v) && isObj(v.candidate) && v.candidate.state === 'confirmed') return 'BREAKOUT';
  return 'FLAG';
}
...
/**
 * The order intent for an Open tap on ANY levelled alert (T-7): a ready plan (GOOD /
 * GET IN NOW) sizes and prices exactly as orderIntentFromPlan; an early SETUP, BREAKOUT
 * or tracked-setup candidate uses its own breakout/invalidation/measured-target levels
 * (candidateLevels) as entry/stop/tp1 -- never the live mark. Same risk-block sizing and
 * {error} contract as orderIntentFromPlan (no risk block -> use /order); preflight still
 * enforces every cap, the 3% stop guard and the entry-drift guard against this entry.
 * @returns {{intent:Object, snap:Object, ready:boolean}|{error:string}}
 */
export function orderIntentFromCandidate(v, caps = {}) {
  const lv = candidateLevels(v);
  if (!lv) {
    const forming = isObj(v) && isObj(v.candidate) && ['forming', 'triggering'].includes(v.candidate.state);
    return { error: forming ? 'this flag has no confirmed levels yet (still forming/triggering); use /order' : 'no entry, stop or TP1 on file for this candidate; use /order' };
  }
  const r = isObj(v.candidate && v.candidate.risk) ? v.candidate.risk : null;
  if (!r || r.reason || !isNum(r.suggestedLeverage) || !isNum(r.collateralUsd)) return { error: 'the engine did not size this plan (no risk block); use /order' };
  const suggested = r.suggestedLeverage * r.collateralUsd;
  ...
  const leverage = isNum(caps.maxLeverage) ? Math.min(caps.maxLeverage, r.suggestedLeverage) : r.suggestedLeverage;
  const intent = {
    symbol: v.symbol, direction: lv.direction, sizeUsd, leverage,
    entry: lv.entry, stop: lv.stop, tp1: lv.tp1, ...(isNum(lv.tp2) ? { tp2: lv.tp2 } : {}),
    planId: (v.plan && v.plan.planId) || null, candidateId: v.candidateId, recClass: (v.rec && v.rec.class) || null, source: 'telegram'
  };
  return { intent, snap: { ...candidateSnapshot(v.symbol, v.s, v.candidateId), symbol: v.symbol }, ready: lv.ready };
}
 ...
 */
export function formatTicketCard(intent, pf, ticket, { mode = null, timeframe = null, nowMs = Date.now(), from = null } = {}) {
  const i = isObj(intent) ? intent : {};
  ...
  // T-8: risk/exposure line (wallet-aware risk policy, lib/execution/riskPolicy.js) and,
  // when the intent's own size exceeds the policy's suggestion, a "suggested $X" note.
  const riskRow = isNum(o.riskUsd) && isNum(o.riskPct)
    ? [['risk', `${fmtUsd(o.riskUsd)} (${fmtPct(o.riskPct)} eq)${isNum(o.exposurePct) ? ` · exposure ${isNum(o.exposurePctBefore) ? `${fmtPct(o.exposurePctBefore)} → ` : ''}${fmtPct(o.exposurePct)}` : ''}`]]
    : [];
  const suggested = isNum(o.suggestedSizeUsd) && isNum(size) && size > o.suggestedSizeUsd ? `\nsuggested ${fmtUsd(o.suggestedSizeUsd)} at the current risk policy.` : '';
  const parts = [
  ...
  if (isObj(from) && from.kind) {
    parts.push(`from ${escapeHtml(from.kind)} ${escapeHtml(o.symbol || i.symbol || '')} ${escapeHtml(from.timeframe || timeframe || '')} · ${escapeHtml(from.ref || '')}`.replace(/\s+/g, ' ').trim());
  }
  parts.push(`ticket <code>${escapeHtml(t.nonce || 'n/a')}</code> · expires in ${secs} s\nTap Confirm, then reply /confirm NONCE PIN.${suggested}`);
...
export function formatEmergencyCloseCard(result, info = {}, { mode = null, timeframe = null } = {}) {
  ...
  return joinSections([
    ...
    codeBlock([
      ...
      ...(typeof r.stopsError === 'string' && r.stopsError ? [['why', r.stopsError.slice(0, 220)]] : []),
...
/**
 * @param {Object} status - executor.status() result
 * @param {Object} [env]
 * @param {{prefs:Object, livePositions:Object}|null} [tg] - Telegram state (T-7 focus row); omitted -> 'auto (no position)'
 */
export function formatExecStatus(status, env = {}, tg = null) {
  ...
  const policy = isObj(risk.policy) ? risk.policy : {};
  const drawdown = isObj(risk.drawdown) ? risk.drawdown : {};
  return joinSections([
    `${killOn ? '🛑' : mode === 'live' ? '🔴' : '🧪'} <b>EXEC</b> · ${mode === 'live' ? 'LIVE' : mode === 'dry' ? 'DRY RUN' : 'MODE UNKNOWN'}${killOn ? ' · KILLED' : ''}`,
    codeBlock([
      ['mode', mode || 'n/a'],
      ['kill', killOn ? `ON${killSrc ? ` (${killSrc})` : ''}` : 'off'],
      ['focus', formatFocusState(isObj(tg) ? tg.prefs : null, isObj(tg) ? tg.livePositions : null)],
      ...
      ['equity', isNum(risk.equityUsd) ? `${fmtUsd(risk.equityUsd)}${isNum(risk.equityAgeSec) ? ` (${risk.equityAgeSec}s old)` : ''}` : 'n/a'],
      ['exposure', fmtPct(risk.exposurePct)],
      ['drawdown', `${fmtPct(drawdown.dayPct)} day · ${fmtPct(drawdown.weekPct)} week`],
      ['policy', `${fmtPct(policy.pctPerTrade)}/trade · ${fmtPct(policy.maxExposurePct)} exposure · ${fmtPct(policy.dailyDrawdownPct)}/${fmtPct(policy.weeklyDrawdownPct)} dd`]
    ]),
    '/kill stops execution now · /arm PIN clears /kill (an env kill stays) · /risk shows the policy · mode is env-only · /alerts focus auto|off changes the focus row'
  ]);
}

/** `/risk`: the effective policy (env defaults + any prefs override), equity, exposure, drawdown. */
export function formatRiskStatus(status, prefsRisk = {}) {
  const s = isObj(status) ? status : {};
  const risk = isObj(s.risk) ? s.risk : {};
  const policy = isObj(risk.policy) ? risk.policy : {};
  const drawdown = isObj(risk.drawdown) ? risk.drawdown : {};
  const overrides = normalizeRiskPrefs(prefsRisk);
  const row = (label, key) => [label, `${fmtPct(policy[key])}${key in overrides ? ' (owner override)' : ' (env default)'}`];
  return joinSections([
    `⚖️ <b>RISK POLICY</b>`,
    codeBlock([
      row('per trade', 'pctPerTrade'),
      row('exposure', 'maxExposurePct'),
      row('per symbol', 'maxPerSymbolPct'),
      row('daily dd', 'dailyDrawdownPct'),
      row('weekly dd', 'weeklyDrawdownPct'),
      ['min gas', isNum(policy.minFreeGasSol) ? `${policy.minFreeGasSol} SOL${'minFreeGasSol' in overrides ? ' (owner override)' : ' (env default)'}` : 'n/a'],
      ['equity', isNum(risk.equityUsd) ? fmtUsd(risk.equityUsd) : 'n/a'],
      ['exposure now', fmtPct(risk.exposurePct)],
      ['drawdown', `${fmtPct(drawdown.dayPct)} day · ${fmtPct(drawdown.weekPct)} week`]
    ]),
    'Usage: /risk pct|exposure|symbolexposure|dailydd|weeklydd|gas VALUE · /risk reset\nAn override can only tighten a knob (never above the env default, never above 2% per trade).'
  ]);
}
