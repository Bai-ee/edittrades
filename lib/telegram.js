 ...
 */
export function formatBreakoutAlert(symbol, c, plan = null) {
  ...
  for (const k of Object.keys(syms).sort()) {
    ...
    if (dataOk) {
      for (const c of Array.isArray(s.candidateSetups) ? s.candidateSetups : []) {
        ...
        alerts.push({ kind: 'BREAKOUT', symbol: k, text: formatBreakoutAlert(k, c, own), replyMarkup: { inline_keyboard: [tradeButtonRow(k, c.timeframe, c.candidateId)] } });
        const snap = {
...
export default {
  ...
  formatBreakoutAlert, formatAlertPrefs, fmtQuiet, parseAlertsArgs, parseMenuLabel, menuKeyboard, chartsKeyboard, alertsKeyboard, shortRef,
