 ...
 *   1. Pure formatters: context payload -> Telegram HTML messages. Alerts are two lines:
 *      the flag (kind · symbol tf direction · brk · void · meas) and one bold verdict
 *      (GET IN NOW | BE READY | WAIT | STAND DOWN) with a plain reason.
...
const stopTp = (stop, tp1) => ` · stop ${lvl(stop)} · TP1 ${lvl(tp1)}`;
 ...
 * eta = minutes to the next close of that timeframe from asOf.
 */
export function alertVerdict(kind, c, { plan = null, rec = null, asOf = null } = {}) {
  ...
  if (own && own.status === 'ready') return `${bold('GET IN NOW')} — retest held · ${goLevels(own)}`;
...
/** /signals: one verdict line per symbol (GOOD first, then BTC/ETH/SOL), its SETUP line when any, then DATA. */
export function formatSignals(payload, nowMs = null) {
  const syms = payload && isObj(payload.symbols) ? payload.symbols : {};
  if (!payload || payload.dataStatus === 'unavailable' || !Object.keys(syms).length) {
    return `<b>STAND DOWN</b> · BTC / ETH / SOL · market data unavailable\n\n${formatDataBlock(payload || {}, nowMs)}`;
  }
  ...
  const lines = [...goods, ...order.filter((k) => !goods.includes(k))].flatMap((k) => {
    const setup = formatSetupBrief(k, syms[k].flagRecommendation && syms[k].flagRecommendation.setup);
    return [formatSignalLine(k, syms[k]), ...(setup ? [setup] : [])];
  });
  return `${lines.join('\n')}\n\n${formatDataBlock(payload, nowMs)}`;
}
...
/**
 * The GOOD alert (sent with the plan-timeframe chart), two lines:
 * "GOOD · ETH 1m LONG · brk X · void Y · meas 3.2R" /
 * "<b>GET IN NOW</b> — retest held · entry · stop · TP1 (R) · net". `test` adds a TEST
 * label line so a sample can never be mistaken for a call.
 */
export function formatGoodAlert(symbol, s, payload = null, { test = false } = {}) {
  ...
  const lines = [alertHeadLine('GOOD', symbol, c, rec), `${bold('GET IN NOW')} — retest held · ${goLevels(plan)}`];
  return `${test ? '🧪 TEST — NOT A SIGNAL\n' : ''}${lines.join('\n')}`;
}

/**
 * SETUP alert, the BREAKOUT shape: "SETUP · SYM tf DIR · brk · void · meas" (levels from
 * the setup's candidate when it is in `candidates`) / "<b>BE READY (Xm)</b> — trigger ·
 * stop · TP1".
 */
export function formatSetupAlert(symbol, setup, rec = null, { plan = null, asOf = null, candidates = [] } = {}) {
  const c = candidateFor(candidates, setup.candidateId, setup);
  const r = { ...(isObj(rec) ? rec : {}), setup };
  return [alertHeadLine('SETUP', symbol, c, r), alertVerdict('SETUP', c, { plan, rec: r, asOf })].join('\n');
}

/**
 * WATCH / TRIGGERING alert, no chart:
 * "WATCH · BTC 3m LONG forming · brk 84,900.00 · void 84,722.60 · meas 2.4R" /
 * "<b>WAIT (2m)</b> — needs a 3m close above 84,900.00, then a retest that holds".
 */
export function formatWatchAlert(symbol, c, rec, { asOf = null } = {}) {
  const kind = c.state === 'triggering' ? 'TRIGGERING' : 'WATCH';
  return [alertHeadLine(kind, symbol, c, rec), alertVerdict(kind, c, { asOf })].join('\n');
}
...
/**
 * BREAKOUT (a candidate first reached confirmed), two lines:
 * "BREAKOUT · SOL 1m SHORT · brk 117.29 · void 117.62 · meas 3.1R · counter-trend (td:bull 4/4)" /
 * "<b>BE READY (1m)</b> — no chase; enter on a retest of 117.29 that holds below · stop · TP1".
 * `symbolPlan` is the symbol's live plan; it drives the verdict only when it is this candidate's.
 */
export function formatBreakoutAlert(symbol, c, symbolPlan = null, rec = null, { asOf = null } = {}) {
  return [alertHeadLine('BREAKOUT', symbol, c, rec), alertVerdict('BREAKOUT', c, { plan: symbolPlan, rec, asOf })].join('\n');
}
  ...
  for (const k of Object.keys(syms).sort()) {
    ...
    if (dataOk && next.setupId && !next.setupIds.includes(next.setupId)) {
      ...
      if (coveredSetups.has(next.setupId)) {
        ...
        state.watch.sigs = rememberSig(state.watch.sigs, sig, 'SETUP', nowMs);
        alerts.push({ kind: 'SETUP', symbol: k, text: formatSetupAlert(k, rec.setup, rec, { plan, asOf, candidates: s.candidateSetups }), replyMarkup: { inline_keyboard: [tradeButtonRow(k, rec.setup.timeframe, next.setupId)] } });
    ...
    if (dataOk && allow('WATCH')) {
      for (const c of Array.isArray(s.candidateSetups) ? s.candidateSetups : []) {
        ...
        if (!fire) continue;
        alerts.push({ kind, symbol: k, text: formatWatchAlert(k, c, rec, { asOf }) });
