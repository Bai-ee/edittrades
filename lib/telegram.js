...
export const COMMANDS = Object.freeze(['start', 'help', 'menu', 'signals', 'why', 'flags', 'wallet', 'journal', 'status', 'chart', 'charts', 'log', 'testalert', 'alerts', 'tracking', 'positions', 'market',
  'order', 'confirm', 'stops', 'exec', 'kill', 'arm', 'mode', 'focus']);
...
export function normalizePrefs(raw) {
  ...
  return { level, quiet, alertTimeframes, focus };
}
...
export function formatExecStatus(status, env = {}, tg = null) {
  ...
  return joinSections([
    ...
    '/kill stops execution now · /arm PIN clears /kill (an env kill stays) · mode is env-only · /alerts focus auto|off changes the focus row'
  ]);
}
