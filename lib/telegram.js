...
/**
 * Short STAND DOWN reason for a rejected plan (never the long remedy sentence):
 * rr_below_min -> "1.2R to first level (needs 2.5R)", room_at_entry -> "entry X inside
 * resistance", chase -> "ran past breakout; wait for retest of X", stop cap -> "stop
 * 3.4% > 3% cap", anything else -> the reasonCode.
 */
export function shortStandDownReason(plan, room = null) {
  const p = isObj(plan) ? plan : {};
  const up = p.direction !== 'short';
  switch (p.reasonCode) {
    case 'rr_below_min': {
      const r = isNum(p.grossRR) ? p.grossRR : (isObj(room) && isNum(room.r) ? room.r : null);
      return `${r === null ? 'n/a' : Math.round(r * 100) / 100}R to first level (needs ${MIN_GROSS_RR}R)`;
    }
    case 'room_at_entry':
      return `entry ${fmtLevel(p.entry)} inside ${up ? 'resistance' : 'support'}`;
    case 'chase':
      return `ran past breakout; wait for retest of ${fmtLevel(p.entry)}`;
    ...
    default:
      return String(p.reasonCode || 'rejected');
  }
}

/**
 * Per-alert readiness call, for THAT candidate (not the symbol's live-plan verdict):
 * WATCH -> WAIT (eta), TRIGGERING -> BE READY (eta), SETUP -> BE READY (eta),
 * BREAKOUT -> GET IN NOW (its plan ready) | BE READY (its plan conditional or its SETUP)
 * | STAND DOWN — <short reason> (its plan rejected) | STAND DOWN — another candidate is selected.
 * eta = minutes to the next close of the candidate's timeframe from asOf.
 */
export function alertReadiness(kind, c, { plan = null, rec = null, asOf = null } = {}) {
  const tf = c && c.timeframe;
  if (kind === 'WATCH') return withEta('WAIT', etaToNextClose(asOf, tf));
  if (kind === 'TRIGGERING' || kind === 'SETUP') return withEta('BE READY', etaToNextClose(asOf, tf));
  const own = isObj(plan) && plan.candidateId === (c && c.candidateId) ? plan : null;
  const ownSetup = isObj(rec) && isObj(rec.setup) && rec.setup.candidateId === (c && c.candidateId) ? rec.setup : null;
  if (own && own.status === 'ready') return 'GET IN NOW';
  if ((own && own.status === 'conditional') || ownSetup) return withEta('BE READY', etaToNextClose(asOf, (own && own.status === 'conditional' ? own.timeframe : ownSetup.timeframe) || tf));
  if (own && own.status === 'rejected') return `STAND DOWN — ${escapeHtml(shortStandDownReason(own, rec && rec.room))}`;
  return 'STAND DOWN — another candidate is selected';
}
...
/** SETUP alert: per-alert BE READY (eta to the setup timeframe's next close) + its room line. */
export function formatSetupAlert(symbol, setup, rec = null, { plan = null, asOf = null } = {}) {
  const room = formatRoomLine(ownRoom(rec, plan, setup.candidateId));
  return [
    ...
    alertReadiness('SETUP', setup, { plan, rec, asOf }),
    ...(room ? [room] : [])
  ].join('\n');
}

/**
 * WATCH / TRIGGERING alert, no chart:
 * "WATCH · BTC 3m LONG forming · break 84,466.10 / void 84,331.60 · meas 2.4R ·
 *  room 1.8R to 84,700.00 (15m resistance) · td:bull:3/4" then the per-alert call
 * ("WAIT (2m)" / "BE READY (2m)"). meas = the flag's measured-move R; the room segment
 * shows only when flagRecommendation.room belongs to this candidate. The td segment is
 * the symbol's top-down reason code, when present.
 */
export function formatWatchAlert(symbol, c, rec, { plan = null, asOf = null } = {}) {
  ...
  const parts = [
    ...
    isNum(c.measuredRR) ? `meas ${Math.round(c.measuredRR * 10) / 10}R` : 'meas n/a'
  ];
  const room = ownRoom(rec, plan, c.candidateId);
  if (room && isNum(room.r) && isNum(room.levelPrice)) parts.push(`room ${fmtR(room.r)} to ${fmtLevel(room.levelPrice)} (${escapeHtml(room.levelSource || 'level')})`);
  if (td) parts.push(escapeHtml(td));
  return [parts.join(' · '), alertReadiness(c.state === 'triggering' ? 'TRIGGERING' : 'WATCH', c, { plan, rec, asOf })].join('\n');
}
 ...
 * `symbolPlan` is the symbol's live plan; its status/call apply only when it is this candidate's.
 */
export function formatBreakoutAlert(symbol, c, symbolPlan = null, rec = null, { asOf = null } = {}) {
  const plan = isObj(symbolPlan) && symbolPlan.candidateId === c.candidateId ? symbolPlan : null;
  ...
  return [
    ...
    status
  ].join(' · ') + `\n${alertReadiness('BREAKOUT', c, { plan, rec, asOf })}`
    + (() => { const room = formatRoomLine(ownRoom(rec, symbolPlan, c.candidateId)); return room ? `\n${room}` : ''; })();
}
...
/**
 * Alert signature `symbol|timeframe|direction|round(breakoutLevel,2)|round(invalidation,2)`
 * (a SETUP without those fields uses entry/stop). Null when the levels are missing.
 */
export function alertSignature(symbol, c) {
  if (!isObj(c)) return null;
  const brk = round2(isNum(c.breakoutLevel) ? c.breakoutLevel : c.entry);
  const inv = round2(isNum(c.invalidation) ? c.invalidation : c.stop);
  if (brk === null || inv === null) return null;
  return `${symbol}|${c.timeframe}|${c.direction}|${brk}|${inv}`;
}

/** The live (< SIGNATURE_TTL_MS old) memory entry for `sig`, else null. */
function liveSig(sigs, sig, nowMs) {
  const e = sig ? sigs.find((x) => x.sig === sig) : null;
  return e && nowMs - Date.parse(e.at) < SIGNATURE_TTL_MS ? e : null;
}
...
export function signatureBlocked(sigs, sig, kind, nowMs) {
  const e = liveSig(sigs, sig, nowMs);
  return !!e && e.kinds.some((k) => SIG_BLOCKED_BY[kind].includes(k));
}

function rememberSig(sigs, sig, kind, nowMs) {
  if (!sig) return sigs;
  const e = liveSig(sigs, sig, nowMs);
  const kinds = e ? [...new Set([...e.kinds, kind])] : [kind];
  return [...sigs.filter((x) => x.sig !== sig), { sig, at: new Date(nowMs).toISOString(), kinds }].slice(-SIGNATURE_RECENT);
}
 ...
 *                 tf, direction, breakout, invalidation) for 60 min: a WATCH never repeats
  ...
  for (const k of Object.keys(syms).sort()) {
    ...
    if (dataOk) {
      for (const c of Array.isArray(s.candidateSetups) ? s.candidateSetups : []) {
        ...
        alerts.push({ kind: 'BREAKOUT', symbol: k, text: formatBreakoutAlert(k, c, plan, rec, { asOf }), replyMarkup: { inline_keyboard: [tradeButtonRow(k, c.timeframe, c.candidateId)] } });
        const snap = {
    ...
    if (dataOk && next.setupId && !next.setupIds.includes(next.setupId)) {
      ...
      if (allow('SETUP') && !signatureBlocked(state.watch.sigs, sig, 'SETUP', nowMs)) {
        state.watch.sigs = rememberSig(state.watch.sigs, sig, 'SETUP', nowMs);
        alerts.push({ kind: 'SETUP', symbol: k, text: formatSetupAlert(k, rec.setup, rec, { plan, asOf }), replyMarkup: { inline_keyboard: [tradeButtonRow(k, rec.setup.timeframe, next.setupId)] } });
    ...
    if (dataOk && allow('WATCH')) {
      for (const c of Array.isArray(s.candidateSetups) ? s.candidateSetups : []) {
        ...
        const sigEntry = liveSig(state.watch.sigs, sig, nowMs);
        // Escalation: forming -> triggering on the same id, or on the same signature.
        const escalation = kind === 'TRIGGERING' && ((seen && seen.state === 'forming') || (sigEntry && sigEntry.kinds.includes('WATCH')));
        ...
        if (!fire) continue;
        alerts.push({ kind, symbol: k, text: formatWatchAlert(k, c, rec, { plan, asOf }) });
