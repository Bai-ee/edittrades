...
/**
 * F1 item 6: turn a flag candidate's internal candle-count offsets
 * (`durationCandles`, `impulseStartCandlesAgo`, `failedAtCandlesAgo`) into the published
 * stateless identity - `candidateId` and `firstDetectedAt`, plus (failed only)
 * `failedAt` - then drops the raw offsets. Coils (type `coil`) are left untouched: they
 * merge two flags with different impulses, so there is no single "impulseStart" to name
 * one by, the same reason they already drop poleHeight/measuredTarget/measuredRR.
 *
 * Fully derived from this request's own candle window - nothing is stored between
 * requests - so the same flag run always gets the same id and timestamps as long as the
 * underlying closed candles haven't rolled off the fetch window.
 *
 * Byte-budget note (F1 item 9, plan Verification): the plan names `impulseStart` and
 * `impulseEnd` as separate published fields alongside `candidateId`; both are
 * mechanically redundant with fields kept here (`impulseStart` is `candidateId`'s own
 * last colon-segment; `impulseEnd` is always exactly `firstDetectedAt` minus one candle
 * of the timeframe) and publishing them anyway pushed the live default payload to
 * ~79.3-79.9 KB, over the 79,000-byte cap. They are dropped from the payload for that
 * reason - the same call already made for `poleHeight` (phase 4) and `window.from`
 * (phase 11) - documented here rather than silently.
 *
 * @param {Object} candidate - a detectFlagLifecycle(...).candidate, timeframe already set
 * @param {{symbol:string, closedThroughIso:string|null, intervalMs:number}} ctx
 * @returns {Object} a new object; `candidate` is never mutated
 */
export function identifyCandidate(candidate, { symbol, closedThroughIso, intervalMs }) {
  if (!candidate || candidate.type !== 'flag') return candidate;
  const { impulseStartCandlesAgo, failedAtCandlesAgo, ...rest } = candidate;
  if (!isFiniteNumber(intervalMs) || typeof closedThroughIso !== 'string') return rest;

  const closedThroughMs = Date.parse(closedThroughIso);
  if (!isFiniteNumber(closedThroughMs)) return rest;
  const isoAt = (candlesAgo) => (isFiniteNumber(candlesAgo) ? new Date(closedThroughMs - candlesAgo * intervalMs).toISOString() : null);

  const firstDetectedAt = isoAt(candidate.durationCandles);
  const impulseStart = isoAt(impulseStartCandlesAgo);

  const out = { ...rest, candidateId: impulseStart ? `${symbol}:${candidate.timeframe}:${candidate.direction}:${impulseStart}` : null, firstDetectedAt };
  if (candidate.state === 'failed') out.failedAt = isoAt(failedAtCandlesAgo);
  return out;
}
...
export default { geometryTimeframeFor, geometryLevels, snapCandidateLevels, resolveCoils, buildVisualGate, identifyCandidate };
