...
export function buildClarity({ candidate, topDown = null, divergence = null, geometryContext = null, minRR = ENGINE_CONFIG.flagPlan.minRR }) {
  ...
  // `minRR` travels with the gate so a reader that cannot import engine config (Telegram)
  // judges its own fallback candidates by the same floor.
  const gate = { passable: !dead && blockers.length === 0, blockers, text: texts.length ? texts.join('; ') : null, minRR: isFiniteNumber(minRR) ? minRR : null };
  ...
  // Context: the non-blocking qual codes in words (blockers already sit in gate.text, which
  // the verdict line carries), then top-down and divergence.
  const context = reasons.filter((code) => !blockers.includes(code)).map(qualWords);
