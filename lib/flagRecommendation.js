...
export function buildFlagRecommendation({
  ...
  if (flagTradePlan.status === 'rejected') {
    const hardBad = ['invalid_levels', 'chase', 'room_at_entry', 'stop_distance_exceeds_cap', 'net_rr_below_3'];
    if (hardBad.includes(flagTradePlan.reasonCode)) {
      opposes.push(reason(flagTradePlan.reasonCode, `The engine rejected the flag plan: ${flagTradePlan.reasonCode}.`, [ref('flagTradePlan.reasonCode')]));
      if (flagTradePlan.reasonCode === 'net_rr_below_3') {
        opposes.push(reason('net_rr', `Net R:R is ${flagTradePlan.netRR}, below the ${minNetRR}R floor.`, [ref('flagTradePlan.netRR')]));
      }
      changeConditions.push(reason('new_valid_plan', 'A fresh flag plan must pass levels, stop distance, and net R:R checks.', [ref('flagTradePlan')]));
  ...
  if (flagTradePlan.status === 'conditional') {
    supports.push(reason('valid_conditional_plan', `The engine has a valid conditional ${flagTradePlan.direction} flag plan with TP1 ${flagTradePlan.tp1} and net R:R ${flagTradePlan.netRR}.`, [ref('flagTradePlan')]));
  ...
  if (isFiniteNumber(flagTradePlan.netRR) && flagTradePlan.netRR >= minNetRR) {
    supports.push(reason('net_rr_ok', `Net R:R to TP1 is ${flagTradePlan.netRR}, meeting the ${minNetRR}R floor.`, [ref('flagTradePlan.netRR')]));
  } else {
    opposes.push(reason('net_rr_unknown_or_low', 'The ready plan does not publish sufficient net R:R.', [ref('flagTradePlan.netRR')]));
    return finish('BAD', 'net_rr_unknown_or_low');
  }
  ...
  changeConditions.push(reason('call_changes_on_invalidation', `Call changes if price invalidates the plan at ${flagTradePlan.stop}, TP1 becomes blocked below 3R, or required data goes stale.`, [ref('flagTradePlan.stop'), ref('flagTradePlan.netRR')]));
