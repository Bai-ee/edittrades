...
export function buildFlagRecommendation({
  ...
  // T6 phase 1: net_rr_ok / fees_heavy (owner decision D1). When the net gate is on
  // (minNetRR set), a plan that reached here already cleared it in the engine - this is
  // confirmation, not a new check. When it's off (null), this is the old informational
  // read against the gross floor, kept for that code path (scripts/replay-rules.js's
  // pre-net-gate variants; production ships with the gate on).
  const netFloor = isFiniteNumber(minNetRR) ? minNetRR : minRR;
  if (isFiniteNumber(flagTradePlan.netRR) && flagTradePlan.netRR >= netFloor) {
    supports.push(reason('net_rr_ok', `Net R:R after fees is ${flagTradePlan.netRR}, meeting the ${netFloor}R floor.`, [ref('flagTradePlan.netRR')]));
  } else {
  ...
  function finish(klass, primaryCode, scored = null) {
    ...
    return {
      ...
      setup: flagTradePlan ? flagTradePlan.setup ?? null : null,
...
export default { buildFlagRecommendation, compactRecommendation, selectWatchCandidate };
