...
function rejectionRemedyText(plan, minRR, maxStopPct) {
...
export function buildFlagRecommendation({
  ...
  if (flagTradePlan.status === 'rejected') {
    ...
    if (hardBad.includes(flagTradePlan.reasonCode)) {
      ...
      changeConditions.push(reason('new_valid_plan', rejectionRemedyText(flagTradePlan, minRR, maxStopPct), [ref('flagTradePlan')]));
