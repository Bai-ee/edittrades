 ...
 * - exact entry condition, final entry/stop/TP1(/TP2), net R:R after fees, and an
 * explicit ready/conditional/rejected status with a stable reason code. It never adds
...
function buildPlanAttempt(candidate, ctx, cfg = ENGINE_CONFIG) {
  ...
  if (netRR === null || netRR < cfg.flagPlan.minNetRR) {
    return { ...shape, reasonCode: 'net_rr_below_3', entry: roundN(entry, 2), stop: roundN(stop, 2), tp1: roundN(tp1, 2), tp2: roundN(tp2, 2), stopDistancePct, netRR };
  }
