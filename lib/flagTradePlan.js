 ...
 * - exact entry condition, final entry/stop/TP1(/TP2), gross R:R (the gate), net R:R
 * after fees (information only), and an explicit ready/conditional/rejected status
 * with a stable reason code. It never adds
...
const FLAG_TF_RANK = { '1m': 0, '3m': 1, '5m': 2 };
 ...
 * @param {Object|null} geometryContext - the symbol's geometryContext (all timeframes)
 * @returns {{touchesEntry:boolean, nearestEdge:number|null}}
 */
function nearestRoomAhead(direction, entry, measuredTarget, geometryContext) {
  ...
  for (const g of Object.values(geometryContext || {})) {
    ...
    for (const z of zones) {
      ...
      if (z.low <= entry && z.high >= entry) touchesEntry = true;
...
/**
 * Net-of-fees R:R for one leg: round-trip fee+slippage (one fill in, one fill out,
 * each costing `feeBps + slippageBps`) is charged against the reward and added to the
 * risk, both as a fraction of `entry`. Never adjusts `entry` or `stop` themselves -
 * "never move the stop closer simply to pass either test" (work package 2, item 4).
 *
 ...
 * `scripts/tracker/flag-paths.js`'s vendored copy of it. No logic change.
 * @param {number} entry
 * @param {number} stop
 * @param {number} target
...
export function observeRetestHold({ direction, entry, candles, fromMs, currentPrice, atrValue, toleranceAtr }) {
  ...
  // Any closed candle after the breakout that reached the level and closed on the hold
  // side is the observed retest-hold. It does not have to be the latest close: once seen,
  // the plan stays ready while the latest close is still on the hold side (chase risk is
  // the candidate's own field, checked separately). A later close back through the level
  // returns the plan to awaiting_retest.
...
function buildPlanAttempt(candidate, ctx, cfg = ENGINE_CONFIG) {
  ...
  const { touchesEntry, nearestEdge } = nearestRoomAhead(direction, entry, measuredTarget, geometryContext);
  if (touchesEntry) {
  ...
  // after fees is published as information only and never rejects a plan.
  const grossRR = roundN(Math.abs(tp1 - entry) / Math.abs(entry - stop), 3);
  const netRR = roundN(netRiskReward(entry, stop, tp1, cfg.risk), 3);
  if (grossRR === null || grossRR < cfg.flagPlan.minRR) {
    return { ...shape, reasonCode: 'rr_below_min', entry: roundN(entry, 2), stop: roundN(stop, 2), tp1: roundN(tp1, 2), tp2: roundN(tp2, 2), stopDistancePct, grossRR, netRR };
  }

  // T6 phase 0 net gate (docs/MASTER_PLAN_T6_FEE_AWARE_FLAGS.md): shipped off
  // (cfg.flagPlan.minNetRR === null), so production is unchanged. A replay variant that
  // sets it rejects here, after the gross gate, with levels kept for the report.
  if (isFiniteNumber(cfg.flagPlan.minNetRR) && (netRR === null || netRR < cfg.flagPlan.minNetRR)) {
    return { ...shape, reasonCode: 'net_rr_below_min', entry: roundN(entry, 2), stop: roundN(stop, 2), tp1: roundN(tp1, 2), tp2: roundN(tp2, 2), stopDistancePct, grossRR, netRR };
  }
  ...
  const { status, reasonCode } = observeRetestHold({ direction, entry, candles, fromMs, currentPrice, atrValue, toleranceAtr: tolAtr });
 ...
 * @returns {Object|null}
 */
export function buildFlagTradePlan({ candidateSetups, geometryContext, tfEntries, marketByTf, candlesByTf, intervalMsByTf, geometryTimeframes, now, configVersion }, cfg = ENGINE_CONFIG) {
  ...
  const attempts = pool.map((candidate) => {
    const market = marketByTf[candidate.timeframe] || {};
    return buildPlanAttempt(candidate, {
      geometryContext,
