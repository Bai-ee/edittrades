 ...
 * (costR >= 0.5) vs the plainer `net_rr_below_min`.
 * @param {number} entry
 * @param {number} stop
 * @param {{feeBps:number, slippageBps:number}} riskCfg
 * @returns {number|null}
 */
export function costRFraction(entry, stop, riskCfg) {
  ...
  return roundN((entry * roundTripCostPct) / risk, 3);
}
...
function buildPlanAttempt(candidate, ctx, cfg = ENGINE_CONFIG) {
  ...
  const costR = costRFraction(entry, stop, cfg.risk);
