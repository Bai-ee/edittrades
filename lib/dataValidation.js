...
export function validateTimeframeAnalysis(tfData, currentPrice) {
  ...
  // Validate liquidity zones - Always ensure it's an array
  if (validated.liquidityZones !== undefined) {
    validated.liquidityZones = Array.isArray(validated.liquidityZones) 
      ? validateLiquidityZones(validated.liquidityZones)
      : [];
  } else {
    validated.liquidityZones = []; // Ensure it's always present
  }

  // Validate divergences - Always ensure it's an array
  if (validated.divergences !== undefined) {
    validated.divergences = Array.isArray(validated.divergences)
      ? validateDivergences(validated.divergences)
      : [];
  } else {
    validated.divergences = []; // Ensure it's always present
  }
