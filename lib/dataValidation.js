...
export function validateMarketStructure(marketStructure) {
  ...
  if (fixed.lastBos) {
    // If type is 'none', direction must also be 'none'
    if (fixed.lastBos.type === 'none') {
      fixed.lastBos.direction = 'none';
    } else if (!['bullish', 'bearish', 'none'].includes(fixed.lastBos.direction)) {
      fixed.lastBos.direction = 'none';
    }
  }
  ...
  if (fixed.lastChoch) {
    // If type is 'none', direction must also be 'none'
    if (fixed.lastChoch.type === 'none') {
      fixed.lastChoch.direction = 'none';
    } else if (!['bullish', 'bearish', 'none'].includes(fixed.lastChoch.direction)) {
      fixed.lastChoch.direction = 'none';
    }
  }
...
export function validateTimeframeAnalysis(tfData, currentPrice) {
  ...
  // Validate volume profile - Always ensure it exists (never null)
  if (validated.volumeProfile) {
    validated.volumeProfile = validateVolumeProfile(validated.volumeProfile, currentPrice);
  } else {
    // Ensure volumeProfile always exists as structured object
    validated.volumeProfile = {
      highVolumeNodes: [],
      lowVolumeNodes: [],
      valueAreaHigh: null,
      valueAreaLow: null
  ...
  // Validate volatility - Always ensure it exists with state (never null)
  if (validated.volatility && currentPrice) {
    validated.volatility = validateVolatility(validated.volatility, currentPrice);
  } else {
    // Ensure volatility always exists as structured object with state
    validated.volatility = {
      atr: null,
      atrPctOfPrice: null,
      state: 'normal' // Always set state, never null
    };
  }
  
  // Ensure volatility.state is never null
  if (!validated.volatility.state) {
    validated.volatility.state = 'normal';
  }

  // Validate fair value gaps - Always ensure it's an array
  if (validated.fairValueGaps !== undefined) {
    validated.fairValueGaps = Array.isArray(validated.fairValueGaps)
      ? validateFairValueGaps(validated.fairValueGaps)
      : [];
  } else {
    validated.fairValueGaps = []; // Ensure it's always present
  }

  // Validate market structure - Always ensure it exists (never null)
  if (validated.marketStructure) {
    validated.marketStructure = validateMarketStructure(validated.marketStructure);
  } else {
    // Ensure marketStructure always exists as structured object
  ...
  // Validate liquidity zones - Always ensure it's an array (never null/undefined)
  validated.liquidityZones = (validated.liquidityZones !== undefined && validated.liquidityZones !== null && Array.isArray(validated.liquidityZones))
    ? validateLiquidityZones(validated.liquidityZones)
    : []; // Always ensure it's an array, never null

  // Validate fair value gaps - Always ensure it's an array (never null/undefined)
  validated.fairValueGaps = (validated.fairValueGaps !== undefined && validated.fairValueGaps !== null && Array.isArray(validated.fairValueGaps))
    ? validateFairValueGaps(validated.fairValueGaps)
    : []; // Always ensure it's an array, never null

  // Validate divergences - Always ensure it's an array (never null/undefined)
  validated.divergences = (validated.divergences !== undefined && validated.divergences !== null && Array.isArray(validated.divergences))
    ? validateDivergences(validated.divergences)
    : []; // Always ensure it's an array, never null

  // Ensure volume is always a structured object (never null)
  if (!validated.volume || typeof validated.volume !== 'object') {
    ...
    // Ensure all volume fields exist with defaults
    validated.volume = {
      current: validated.volume.current !== undefined && validated.volume.current !== null ? validated.volume.current : 0,
      avg20: validated.volume.avg20 !== undefined && validated.volume.avg20 !== null ? validated.volume.avg20 : 0,
      trend: validated.volume.trend || 'flat'
    };
  }
