...
export function calculateVolumeProfile(candles) {
  ...
  const hasVolume = candles.every(c => c.volume !== undefined && c.volume !== null);
  if (!hasVolume) {
    return null;
  }
...
export function calculateDivergences(candles, indicators) {
  ...
  // Need RSI or StochRSI history for divergence detection
  // For now, return empty array - can be implemented with proper indicator history
  // Divergence detection requires comparing price action vs indicator action over time
...
export function calculateAllAdvancedChartAnalysis(candles, indicators, swingPoints, trend) {
  ...
  try {
    ...
    if (fairValueGaps && fairValueGaps.length > 0) {
      result.fairValueGaps = fairValueGaps;
    }
  } catch (error) {
