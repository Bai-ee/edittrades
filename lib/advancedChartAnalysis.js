...
export function calculateMarketStructure(candles, swingPoints, trend) {
  ...
  if (!finalBos.type || finalBos.type === 'BOS' && !finalBos.price) {
    finalBos.type = 'none';
  ...
  if (!finalChoch.type || finalChoch.type === 'CHOCH' && !finalChoch.price) {
...
export function calculateVolumeProfile(candles) {
  ...
  const numBins = Math.min(24, Math.max(3, Math.floor(candles.length / 3)));
...
export function calculateDivergences(candles, indicators) {
  ...
  const pivotDepth = Math.min(1, Math.floor(closes.length / 5));
  for (let i = pivotDepth; i < closes.length - pivotDepth; i++) {
    ...
    // If we have more candles, check deeper pivots
    if (pivotDepth >= 2) {
      ...
      // Shallow pivot (only 1 candle on each side)
      pricePivots.push({
        index: i,
        price: closes[i],
