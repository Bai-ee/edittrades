...
- Tests: `test:jupiter` 26 → 36 (landing state machine, `waitForFill`, `buildCancelIncreaseRequest`, `fetchAccountsExist`), `test:execution` 51 → 64 (two-phase open happy path + every failure branch, exactly-once, `listPositions` stops), `test:telegram` 107 → 113 (phase card, emergency-close card, `/positions` stops line). All fourteen `npm run test:*` suites green; `git diff --check` clean.
