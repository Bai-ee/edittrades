...
## 2026-09-26 — Execution T-8: wallet-aware risk policy (`docs/PROMPT_T8_AGENT_H.md`, not deployed, nothing enabled)

Code and tests only; no env change, no deploy. A layer on top of the executor's env caps (`EXECUTION_MAX_*` stay hard floors, unchanged).

- New `lib/execution/riskPolicy.js` (pure, no I/O): `evaluateRiskPolicy({equityUsd, openPositions, dailyPnlUsd, weekPnlUsd, freeGasSol, intent, policy})` computes risk-per-trade, exposure (overall + per symbol), daily/weekly drawdown and a free-gas floor against env defaults (`RISK_PCT_PER_TRADE` 0.5, `RISK_MAX_EXPOSURE_PCT` 25, `RISK_MAX_PER_SYMBOL_PCT` 15, `RISK_DAILY_DRAWDOWN_PCT` 3, `RISK_WEEKLY_DRAWDOWN_PCT` 8, `RISK_MIN_FREE_GAS_SOL` 0.05), returning reasons (`risk_pct_over`, `exposure_over`, `symbol_exposure_over`, `daily_drawdown`, `weekly_drawdown`, `gas_low`, `equity_unavailable`), a suggested size/leverage and exposure before/after (mirrored long/short). `applyRiskPrefs` bounds an owner override tighten-only (never above the env default, `pctPerTrade` never above 2% absolute).
- `lib/execution/executor.js`: equity = the SIGNING wallet's own SOL + stablecoin value (`services/walletTracker.js` `getAccountSnapshot`, pointed at the executor's own address — never `TRACKED_WALLET_ADDRESS`, never a key; `walletTracker.js` itself is unchanged) plus open-position collateral ± unrealized PnL, cached 60 s. `preflight` calls the policy after the env caps, before the market/custody/quote calls — its reasons refuse like any gate; the order carries the risk/exposure/suggested-size fields and the audit `preflight` line carries a `risk:{}` block. A daily/weekly drawdown breach also engages the kill switch (`risk_daily_drawdown` / `risk_weekly_drawdown`); `/arm` clears it, the env kill still wins. `dailyLossUsd` generalized into `pnlWindow(store, nowMs, days)` (kept as a thin wrapper) for the weekly read. New `riskPrefBound(key)` (sync, side-effect-free) lets the webhook validate a `/risk` override without importing `lib/execution/riskPolicy.js` directly.
- Telegram (`lib/telegram.js`, `api/telegram-webhook.js`): the order ticket gets a `risk $X (Y% eq) · exposure B% → A%` line and a `suggested $X` note when the intent's size exceeds the policy's suggestion (Open uses the suggestion unless the owner typed an explicit size). `/exec` adds equity, exposure, drawdown day/week and a policy line. New `/risk` shows the effective policy (env default vs. owner override) and the live snapshot; `/risk pct|exposure|symbolexposure|dailydd|weeklydd|gas VALUE` sets one override, `/risk reset` clears all, written to `state.prefs.risk` (now carried through an unrelated `/alerts` write instead of being dropped). `lib/telegram.js` still imports only `trackStory.js` — a small local `normalizeRiskPrefs` duplicate (not an import) keeps that invariant.
- Tests: new `test-risk-policy.js` (31, `npm run test:riskpolicy`); `test:execution` 64 → 74 (equity source, 60 s cache, `equity_unavailable`, drawdown kill + arm, order/audit risk fields, prefs bounds); `test:telegram` 110 → 120 (ticket/exec/risk rendering, `/risk` end to end, prefs survive `/alerts`). All fourteen `npm run test:*` suites green; `git diff --check` clean.
- Known limits, not yet resolved: `equityPrices()` only prices the SOL holding via the injected mark source (`deps.getMarkPrices` or one engine build, cached alongside equity) — a non-SOL, non-stablecoin holding is never priced into equity, matching `walletTracker.js`'s own "unpriced, never guessed" policy; drawdown is measured against realized PnL only (journal closes + un-journaled live audit closes), not unrealized.
