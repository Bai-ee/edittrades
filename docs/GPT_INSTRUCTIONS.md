...
risk(fields per schema):never exceed maxLeverage;Leverage/Size=suggestedLeverage*collateralUsd;absent/reason set:Leverage provisional(labeled),Wallet Risk/Loss Unavailable.
...
| `flagTradePlan.{candidateId,planId,status,reasonCode,entryType,entryCondition,entry,stop,tp1,tp2,grossRR,netRR,costR,stopDistancePct}` (schema 1.13.0, signal-reliability minimum plan; `grossRR` 1.17.0 is the gross gate, `flagPlan.minRR` - 2.5 as of "D-variant revised" 2026-09-24, was 3.0; `netRR`/`costR` 1.21.0, T6 phase 1, direction-priced since 1.24.0 (T6 completion plan C1) - net gate off in production (`flagPlan.minNetRR` null) as of "D-variant revised"; `net_rr_below_min`/`stop_inside_costs` reachable only under a research override. `flagRecommendation` supports `net_rr_ok` (netRR >= 1.0) or opposes `net_rr_low` (netRR < 1.0, "thin after fees")) | CANDIDATES — "flagTradePlan=trade authority" rule; COMMANDS `trades` |
...
| `flagRecommendation.clarity.{candidateId,gate{passable,blockers,text},killIf{level,text},otherSide{low,high,source,text},context[],divergence{agree,conflict}}` (schema 1.27.0, alert clarity - presentation from `qual`/`ema21Hold`/geometry, never a gate) | CANDIDATES — `gate.text` before any BE READY; Kill if / Other side always printed |
...
| `risk{maxLeverage,suggestedLeverage,lossAtStopUsd,lossAtStopPct,lossAtStopPctOfWallet,collateralUsd,reason}` (field list in `docs/GPT_PLAYBOOK_ADDITIONS_2026-09-24.md` since 1.27.x) | RISK — leverage/sizing rules |
...
| `config.risk.costBpsByDirection.{long,short}` (schema 1.24.0, T6 completion plan C1, D-cost decision, `docs/OWNER_DECISIONS_2026-09-24.md` - positions funded from USDC/USDT, a long pays the extra swap in/out) | drives `flagTradePlan.netRR`/`costR`, `flagRecommendation`'s `net_rr_ok`/`net_rr_low`, and `flagRecommendation.setup.netRR`; not named directly in the box (netRR is no longer a gate to explain a formula for) |
...
- 2026-09-25 (alert clarity, schema 1.27.x, `docs/PLAN_ALERT_CLARITY.md`, no rule/threshold change): CANDIDATES - `flagRecommendation.clarity:gate.text before any BE READY;always print Kill if/Other side.` Funded by the B3 pattern (a field-name list, not a rule): RISK `risk{maxLeverage,suggestedLeverage,lossAtStopUsd,lossAtStopPct,lossAtStopPctOfWallet,collateralUsd,reason}:` → `risk(fields per schema):` (the rule still names `maxLeverage`, `suggestedLeverage`, `collateralUsd` and `reason` where it uses them); the full list moved to `docs/GPT_PLAYBOOK_ADDITIONS_2026-09-24.md` section 4. Header 1.25.x → 1.27.x. 7939 units / 7982 bytes → 7946 units / 7989 bytes.
