...
| M-5b Measured target | Owner stated | Candidate pole height, breakout, invalidation, geometry cap | Engine computes target and caps TP1 before measured move when a major level is first. | `flagTradePlan.tp1/tp2/netRR`; cited verbatim. | <3 net R is BAD | Missing/invalid levels reject plan. |
...
| M-9 Risk management edge | Owner stated | Engine-owned entry/stop/TP1/netRR | Every ready/conditional plan must have at least 3 net R to capped TP1. | `net_rr_ok` support or `net_rr_below_3` BAD. | Yes | Missing levels/data → rejected or DATA_UNAVAILABLE. |
