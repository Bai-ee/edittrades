...
**Superseded by D-variant revised (below the decisions table): the roles here flipped
same-day - v3 (minRR 3.0) is now the shadow, minRR 2.5 is now live. The mechanism
described below (shadowVariants, `include=model`-only, served-call capture) is
unchanged; only the config values and shadow `id` did.**
