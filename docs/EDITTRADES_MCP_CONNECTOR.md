...
Payload: `schemaVersion` 1.8.0, `configVersion` 2026.09.22-6 live; 1.11.0 / 2026.09.22-10 local (Phases 9, 10, 9b, trading-model quick pass Q1-Q3), not deployed
...
| Command | File | Passing (2026-09-23) |
| --- | --- | --- |
| `npm run test:sltp` | `test-strategy-sltp.js` (section 6b = call-path guard tests) | 50 |
| `npm run test:scalp` | `test-scalp-context.js` | 106 |
...
| `npm run test:pattern` | `test-pattern-detector.js` (fixtures `test/fixtures/flagFixtures.js`) | 25 |
...
| `npm run test:replay` | `test-replay.js` (replay harness, metrics, miss log `test/fixtures/misses/`, outcome scoring) | 30 |
| `npm run test:bias` | `test-bias-matrix.js` (bias matrix, alignment, decision inputs, mirrors, td:/a200: trace grammar) | 15 |
| `npm run test:topdown` | `test-top-down.js` (weekly aggregation/lean, top-down vote, above/below-200; trading-model quick pass Q3) | 14 |
