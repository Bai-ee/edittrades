...
**Current product:** EditTrades scalp context — closed-candle BTC/SOL/ETH context, legacy strategy engine, and 21/200 flag recommendation, served to ChatGPT via REST Action (`GET /api/scalp-context`) and MCP (`POST /api/mcp`). Payload schema 1.8.0 live on Vercel; schema 1.21.0 deploying this pass (T6 phase 1, net gate).
