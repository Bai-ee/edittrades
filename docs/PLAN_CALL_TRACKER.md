...
## Telegram alerts + transitions (added 2026-09-25)

`docs/PLAN_TELEGRAM.md` "Sent-alert and transition logs". `collect.js` `pullTelegramLogs` pulls `telegram/alerts/` and `telegram/transitions/` (manifest, then day files from the newest stored day minus one) into `data/telegram-alerts/` (dedupe by id) and `data/transitions/` (dedupe by candidateId+at); lines are rebuilt from a field whitelist, then the usual strip and fail-closed check (`--no-telegram` skips). `score.js` `scoreAlerts` → `data/alert-outcomes.jsonl`: per alert, `latencyMin` = sentAt − the close of the alert timeframe's candle at or before the payload `closedThrough` (1m when unknown); `outcome` = the furthest the candidate got within 24 h after the alert (tp1 | stop from a later TRACK hit or the joined ready plan's walk > get_in_now > setup > confirmed > void > expired; else pending / none); `laterGood`; `readyAfterMin`; the joined scored call (ready plan, then setup, then rec). `aggregate.js` `alerts` block: alerts by kind / verdict / outcome (7d) and per day, median latency, "alerted → later GOOD" (alerts before any GET IN NOW, excluding hit / nudge / ended messages), "BE READY → GET IN NOW within 30 min" (BE READY alerts at least 30 min old), transitions per hour by timeframe (24 h, span capped to logged time). Page: `#zone-alerts` (jump nav `Alerts`; the Status `Alerts` fact links to it) with tiles `#tile-alerts-7d`, `#tile-alert-latency`, `#tile-alert-later-good`, `#tile-be-ready-to-go`, sections `#alerts-mix-section`, `#alerts-transitions-section`, `#alerts-daily-section` (PROVISIONAL). Empty states `[NO SENT ALERTS LOGGED YET]`, `[NO TRANSITIONS LOGGED YET]`. Counts only; nothing tunes a rule. `test:tracker` 111 → 114.
